...
import { Briefcase, HelpCircle, Info, Users } from "lucide-react";
import React from "react";
import { DAYS, DayKey, WeekSchedule, WeekStatus } from "./types";

export interface WeekSchedulerProps {
  readonly value: WeekSchedule;
  readonly onChange?: (value: WeekSchedule) => void;
  readonly readOnly?: boolean;
  readonly labels?: Partial<Record<WeekStatus, string>>;
          ...
          {DAYS.map((day) => {
            ...
            return (
              <div key={day.key} className="flex flex-col items-center">
                <div
                  className={`text-xs sm:text-lg font-bold mb-2 ${day.key === "sun"
                      ? "text-red-500"
                      : day.key === "sat"
                        ? "text-blue-500"
                        : "text-slate-400"
                ...
                <button
                  ...
                  className={`w-full aspect-square max-w-[50px] sm:max-w-[80px] rounded-xl sm:rounded-3xl flex items-center justify-center transition-all transform shadow-sm ${config.color} text-white ${readOnly
