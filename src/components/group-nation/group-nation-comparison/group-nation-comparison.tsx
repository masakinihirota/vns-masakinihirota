    ...
    >
      ...
      <div className="grid md:grid-cols-2 gap-8">
        {COMPARISON_DATA.map((entity) => (
          <Card
            key={entity.id}
            className={`group overflow-hidden transition-all duration-500 hover:shadow-2xl hover:-translate-y-1 ${entity.id === "group"
          ...
          >
            <CardHeader className="relative p-8 border-b border-white/20 dark:border-white/5">
              <div className="flex items-center gap-4">
                <div className={`p-4 rounded-3xl shadow-sm ${entity.id === "group"
                  ...
                  {entity.id === "group" ? <Users size={32} /> : <Globe size={32} />}
                </div>
                <CardTitle className={`text-2xl font-bold ${entity.id === "group" ? "text-rose-700 dark:text-rose-200" : "text-indigo-700 dark:text-indigo-200"
                  }`}>
                  {entity.title}
                </CardTitle>
              </div>
