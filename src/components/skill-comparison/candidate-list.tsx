...
interface CandidateListProps {
  ...
  readonly onToggleAction?: (id: string | number, action: 'followed' | 'watched') => void;
          ...
          if (!showDetails) {
            ...
            return (
              <div key={c.id} className="flex group">
                ...
                {onToggleAction && (
                  <button
                    onClick={() => onToggleAction(c.id, actionType === 'add' ? 'followed' : 'followed')} // 暫定
            ...
            >
              <div className="flex justify-between items-start mb-3">
                <div className="text-base font-black uppercase text-slate-800 tracking-tight flex items-center">
                  <span>{c.name}</span>
                  <span className={`ml-3 text-xs px-2.5 py-1 rounded-full font-black tabular-nums transition-colors ${masteryCount >= 6 ? 'bg-indigo-600 text-white' :
                      masteryCount >= 3 ? 'bg-blue-100 text-blue-600' :
                        'bg-slate-100 text-slate-500'
                    }`}>
                    {masteryCount}/8
                  </span>
                </div>
                <ArrowRightLeft size={18} className={`transition-colors duration-300 ${isSelected ? 'text-indigo-400' : 'text-slate-200 group-hover:text-indigo-300'}`} />
              </div>
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-5 italic truncate line-clamp-1">{c.role}</p>

              <div className="flex items-center justify-between border-t border-slate-50 pt-5">
                <div className="flex -space-x-1.5">
                  {Object.keys(c.mastery || {}).map(s => (
                    <div key={s} className="w-6 h-6 rounded-full bg-slate-100 border-2 border-white flex items-center justify-center shadow-sm" title={s}>
                      <span className="text-[8px] font-bold text-slate-400 capitalize">{s[0]}</span>
                    </div>
                  ))}
                </div>
                {onToggleAction && (
