...
describe("ProductList Logic", () => {
  ...
  describe("sortArtworks", () => {
    it("ratingの昇順でソートできる", () => {
      const result = sortArtworks(MOCK_ARTWORKS, { key: "rating", order: "asc" });
      expect(result[0].rating).toBe("Tier1");
      // 先頭がTier1であることを確認（Tier1は複数あるので、少なくとも最初の要素はTier1）
    });

    it("ratingの降順でソートできる", () => {
      const result = sortArtworks(MOCK_ARTWORKS, { key: "rating", order: "desc" });
    ...
    it("titleでソートできる", () => {
      const result = sortArtworks(MOCK_ARTWORKS, { key: "title", order: "asc" });
    ...
    it("target_ageでソートできる", () => {
      const result = sortArtworks(MOCK_ARTWORKS, { key: "target_age", order: "desc" });
      // 18+ が先頭に来るはず
