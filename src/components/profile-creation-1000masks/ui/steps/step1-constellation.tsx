  ...
  return (
    <section className="bg-slate-50/50 dark:bg-white/5 p-10 rounded-[3rem] border border-slate-100 dark:border-white/10 shadow-sm transition-all text-left">
      <StepHeader
        num="1"
        title="星座匿名（識別ID）"
        subtitle="あなたの幽霊の名前をリストから決めてください。"
        isGhost={activeProfile.isGhost}
      />
      <div className="flex items-center justify-between mb-8 text-left transition-colors duration-300">
        <p className="text-lg text-slate-500 dark:text-slate-400 font-bold leading-relaxed text-left">
          あなたの星座{" "}
          <span className="font-black dark:text-neutral-200">
            [{USER_BASE_CONSTELLATION}]
          </span>{" "}
          の候補
        </p>
        <div className="flex space-x-3 shrink-0 text-left">
          ...
          <button
            ...
            className={`flex items-center space-x-3 px-6 py-3 rounded-2xl text-white font-black text-lg shadow-xl active:scale-95 transition-all ${
              isAtHistoryEnd
                ? activeProfile.isGhost
                  ? "bg-purple-600 dark:bg-purple-700 shadow-purple-100 dark:shadow-none hover:bg-purple-700"
                  : "bg-blue-600 dark:bg-blue-700 shadow-blue-100 dark:shadow-none hover:bg-blue-700"
      ...
      <div className="flex flex-col gap-4 text-left">
        {activeProfile.constellationHistory[activeProfile.historyPointer].map(
          (name, i) => {
            ...
            return (
              <button
                ...
                className={`group p-8 rounded-[2rem] border-2 transition-all text-left relative flex items-center justify-between ${
                  activeProfile.isGhost ? ghostStyles : normalStyles
                }`}
              >
                <div className="flex flex-col text-left">
                  ...
                  <div
                    className={`text-xl font-serif font-black leading-tight ${isSel ? "text-slate-900 dark:text-neutral-100" : "text-slate-700 dark:text-slate-500"}`}
                  >
                    {name}
                  </div>
                </div>
                {isSel && (
                  <div
                    className={`p-2.5 rounded-full ${activeProfile.isGhost ? "bg-purple-600 dark:bg-purple-700" : "bg-blue-600 dark:bg-blue-700"} shadow-lg text-white`}
                  >
