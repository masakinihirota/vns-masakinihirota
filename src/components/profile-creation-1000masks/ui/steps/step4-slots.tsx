      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-left">
        {ALL_SLOTS.map(slot => {
          ...
          return (
            ...
            >
              <div className={isSelected ? 'text-blue-600 dark:text-blue-400' : 'text-slate-300 dark:text-slate-800'}>
                {isSelected ? <CheckCircle2 size={28} /> : <Circle size={28} />}
              </div>
              <div className="flex items-center space-x-3 text-left">
                {React.createElement(slot.icon, {
                  size: 24,
                  className: isSelected ? 'text-blue-500 dark:text-blue-400' : 'opacity-40 dark:opacity-20'
                })}
