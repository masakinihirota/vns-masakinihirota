  ...
  return (
    <header className="sticky top-0 z-50 bg-white/80 dark:bg-[#0B0F1A]/80 backdrop-blur-md px-10 py-6 border-b border-slate-100 dark:border-white/5 flex items-center justify-between transition-colors duration-300">
      <div className="flex-1 mr-8 flex items-center space-x-8 text-left">
        <div className={`p-4 rounded-2xl shadow-lg transition-all duration-500 ${activeProfile.isGhost ? 'bg-purple-600 text-white' : 'bg-blue-600 text-white'}`}>
          <Icon size={32} />
        </div>
        <div className="flex-1 text-left">
          ...
          <p className="text-lg font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em]">{activeProfile.constellationName}</p>
        </div>
      </div>
