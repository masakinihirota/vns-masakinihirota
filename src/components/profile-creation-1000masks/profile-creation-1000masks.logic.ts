...
export type MaskId = 'mask_default' | 'mask_zap' | 'mask_shield' | 'mask_star' | 'mask_target' | 'mask_anchor' | 'mask_cloud' | 'mask_sun' | 'mask_aperture' | 'mask_smile' | 'ghost';
...
export type ProfileTypeId = 'self' | 'interview' | 'imagined' | 'ideal' | 'politician' | 'ai_dummy';
...
export type SlotId = 'works' | 'favorites' | 'values' | 'skills' | 'politics' | 'partner_meta' | 'social';
...
export type ObjectiveId = 'business_card' | 'resume' | 'build_work' | 'play' | 'partner' | 'consult' | 'politics' | 'other';
...
export type ValueCategoryId = 'val_core' | 'val_basic' | 'val_work' | 'val_play' | 'val_partner' | 'val_life' | 'val_ethic' | 'val_end' | 'val_politics';
...
export const COLORS = ["赤い", "青い", "黄色い", "緑の", "紫の", "橙色の", "ピンクの", "茶色の", "灰色の", "黒い", "白い"];
export const MATERIALS = ["マテリアル", "光", "幻想", "氷", "炎", "輝き", "熱狂", "闇", "風", "水", "土", "雷", "音", "光速", "宇宙"];
...
export const PROFILE_TYPES: ProfileType[] = [
  ...
  { id: 'imagined', label: '心象プロフィール (IMAGINED)', icon: Eye, description: '外部の断片的な情報とあなたの想像力で、あなたから見たその人の人物像を構築します。' },
  ...
  { id: 'ai_dummy', label: 'AIダミー生成 (AI DUMMY)', icon: Cpu, description: 'テスト用にAIが架空の設定を自動生成します。デザイン確認や動作テストのために使用します。', isSpecial: true },
];
...
export const OBJECTIVE_PRESETS: ObjectivePreset[] = [
  ...
  { id: 'other', label: 'その他', slots: ['favorites', 'values', 'skills'], icon: Settings },
];
...
export const generateCandidateSet = (constellation: string, previousSet: string[] = []) => {
  ...
  while (newSet.length < 3) {
    ...
    if (!newSet.includes(name) && !previousSet.includes(name)) newSet.push(name);
  }
  return newSet;
};
...
export const useProfileCreation = (adapter?: ProfileStorageAdapter) => {
  ...
  const requestSwitchProfile = (id: string) => {
    ...
    if (isDirty) {
      setModal({ isOpen: true, type: 'confirm_exit', targetId: id, message: '変更が保存されていません。破棄して移動しますか？' });
    } else {
  ...
  const handleSave = () => {
    if (!activeProfile.isGhost) {
      if (!activeProfile.selectedTypeId) {
        setModal({ isOpen: true, type: 'error', targetId: null, message: 'STEP 2: プロフィールのタイプを選択してください。' });
        return;
      }
      if (activeProfile.selectedObjectiveIds.length === 0) {
        setModal({ isOpen: true, type: 'error', targetId: null, message: 'STEP 3: プロフィールの目的を1つ以上選択してください。' });
        return;
      }
    }
  ...
  const handleNextAnonyms = () => {
    ...
    handleUpdateDraft({ constellationHistory: newHistory, historyPointer: newPointer });
  };

  const handlePrevAnonyms = () => {
    if (activeProfile.historyPointer > 0) handleUpdateDraft({ historyPointer: activeProfile.historyPointer - 1 });
  };
  ...
  const handleToggleObjective = (objId: ObjectiveId) => {
    ...
    if (isAdding) {
      if (objId === 'partner') {
        const otherProfileWithPartner = profiles.find(p => p.id !== activeProfile.id && p.selectedObjectiveIds.includes('partner'));
      ...
      if (preset) {
        const newSlotsSet = new Set([...activeProfile.selectedSlots, ...preset.slots]);
        handleUpdateDraft({ selectedObjectiveIds: newObjectiveIds, selectedSlots: Array.from(newSlotsSet) });
      }
    } else {
  ...
  const toggleSlot = (slotId: SlotId) => {
    ...
    if (isCurrentlySelected) {
      const newSlots = activeProfile.selectedSlots.filter(id => id !== slotId);
      const newObjectiveIds = activeProfile.selectedObjectiveIds.filter(objId => {
        const preset = OBJECTIVE_PRESETS.find(p => p.id === objId);
        return preset && !preset.slots.includes(slotId);
      });
      handleUpdateDraft({ selectedSlots: newSlots, selectedObjectiveIds: newObjectiveIds });
    } else {
      handleUpdateDraft({ selectedSlots: [...activeProfile.selectedSlots, slotId] });
    }
  };
  ...
  const createNewProfile = () => {
    const performCreate = () => {
      ...
      if (ghost) {
        const newProfile: Profile = { ...ghost, id: newId, name: '新しい仮面', isGhost: false, avatarType: 'user', maskId: 'mask_default', selectedTypeId: null, selectedObjectiveIds: [], selectedSlots: [], selectedValues: ['val_core'] };
    ...
    if (isDirty) {
      setModal({ isOpen: true, type: 'confirm_create', targetId: null, message: '変更を破棄して、新規作成しますか？', action: performCreate });
    } else {
      performCreate();
    }
  };
  ...
  const filteredProfiles = useMemo(() => {
    return profiles.filter(p => p.name.toLowerCase().includes(searchQuery.toLowerCase()) || p.constellationName.includes(searchQuery));
