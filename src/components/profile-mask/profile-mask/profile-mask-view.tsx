...
import { INITIAL_SKILL_CASSETTES, INITIAL_WORKS_CASSETTES, MASK_ICONS, PROFILE_TYPES } from './profile-mask.constants';
...
export const ProfileMaskView: React.FC<ProfileMaskViewProps> = ({
  ...
  const ActiveIcon = getMaskIcon(activeProfile.maskId, activeProfile.avatarType);
  ...
  return (
    <div className="flex h-screen bg-[var(--bg)] text-[var(--text)] font-sans overflow-hidden">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 dark:bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-[var(--bg)] dark:bg-neutral-900/95 rounded-[2rem] shadow-2xl max-w-lg w-full overflow-hidden border border-white/20 dark:border-white/10 animate-in zoom-in-95 duration-300">
            <div className="p-10 text-center">
              ...
              <h3 className="text-lg font-black text-[var(--text)] mb-3 uppercase tracking-widest">Notification</h3>
              <p className="text-lg font-bold text-slate-500 dark:text-neutral-400 leading-relaxed">{modal.message}</p>
            </div>
            <div className="p-8 bg-black/5 border-t border-white/10 flex items-center justify-center space-x-4">
              {modal.type.startsWith('confirm') ? (
                <>
                  <button onClick={onCloseModal} className="px-8 py-3 rounded-2xl font-black text-lg text-slate-400 dark:text-neutral-500 hover:bg-white/10 transition-all">キャンセル</button>
                  <button onClick={onConfirmModal} className="px-10 py-3 rounded-2xl font-black text-lg bg-[var(--accent)] text-white shadow-xl transition-all active:scale-95">確定</button>
                </>
              ) : (
                <button onClick={onCloseModal} className="px-12 py-3 rounded-2xl font-black text-lg bg-[var(--accent)] text-white shadow-xl transition-all active:scale-95">了解</button>
              )}
            </div>
          </div>
        </div>
      ...
      <aside className="w-[24rem] bg-[var(--card)] backdrop-blur-md border-r border-white/20 dark:border-white/10 flex flex-col shadow-lg shrink-0">
        <div className="p-6 border-b border-white/20 dark:border-white/10 shrink-0">
          <div className="flex items-center space-x-4 mb-8">
            <div className="w-10 h-10 bg-[var(--accent)] rounded-xl flex items-center justify-center text-white font-black italic shadow-lg">V</div>
            <h1 className="text-xl font-black tracking-tighter text-[var(--text)]">1000 Masks</h1>
          </div>
          <button onClick={onCreateNewProfile} className="w-full flex items-center justify-center space-x-3 bg-[var(--accent)] hover:bg-[var(--accent)]/90 text-white py-4 rounded-[1.25rem] shadow-xl shadow-blue-500/20 transition-all active:scale-95 font-black text-lg mb-8">
            <Plus size={20} />
            <span>プロフィール作成</span>
          </button>
            ...
            return (
              ...
              >
                <div className="flex items-center w-full mb-2">
                  <div className={`p-3 rounded-xl mr-5 ${isActive ? 'bg-white/20 text-white' : 'bg-slate-100 dark:bg-white/10 text-slate-400'}`}>
                    <Icon size={24} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className={`font-black truncate text-lg ${isActive ? 'text-white' : 'text-[var(--text)]'}`}>
                      {p.name}
                    </p>
                  </div>
                </div>
                <div className="pl-[4.75rem] w-full">
                  <p className={`text-sm font-black truncate uppercase ${isActive ? 'text-white/70' : 'text-slate-400'}`}>
                    {p.constellationName}
                  </p>
                </div>
              </button>
      ...
      <main className="flex-1 flex flex-col overflow-hidden m-4 bg-[var(--card)] backdrop-blur-md rounded-[2.5rem] border border-white/30 dark:border-white/10 shadow-2xl relative">
        {/* Header */}
        <header className={`p-8 border-b transition-all duration-500 shrink-0 bg-[var(--bg)]/90 backdrop-blur-md z-20 flex justify-between items-center ${isDirty ? 'border-b-amber-400' : 'border-b-slate-100 dark:border-white/5'}`}>
          <div className="flex-1 mr-8 flex items-center space-x-8">
            <div className={`p-6 rounded-[2rem] shadow-inner transition-all duration-500 ${activeProfile.isGhost ? 'bg-purple-600 text-white' : 'bg-[var(--accent)] text-white'}`}>
              <ActiveIcon size={36} />
            </div>
            <div className="flex-1">
              <div className="flex items-center space-x-3 text-slate-400 mb-1.5">
                <span className={`text-sm font-black uppercase tracking-widest ${activeProfile.isGhost ? 'text-purple-500' : 'text-blue-500'}`}>
                  {activeProfile.isGhost ? 'ROOT' : 'MASK'}
                </span>
                <span className="text-slate-200 dark:text-white/10">|</span>
                <span className="text-sm font-black uppercase">{activeProfile.constellationName}</span>
              </div>
              <input
                type="text"
        ...
        {/* Content */}
        <div ref={mainScrollRef} className="flex-1 overflow-y-auto p-12 max-w-7xl mx-auto w-full space-y-24 pb-64 no-scrollbar">
          {/* STEP 1: 星座匿名 */}
          <section className="bg-white/10 dark:bg-white/5 backdrop-blur-md p-10 rounded-[3rem] border border-white/20 dark:border-white/10 shadow-sm transition-all">
            <div className="flex items-start space-x-5 mb-8">
              <div className="w-12 h-12 rounded-[1rem] flex items-center justify-center font-black text-lg shadow-lg shrink-0 bg-[var(--accent)] text-white">1</div>
              <div className="pt-1">
                <h2 className="text-xl font-black text-[var(--text)] tracking-tight leading-none mb-2 uppercase">星座匿名（識別ID）</h2>
                <p className="text-sm text-slate-400 font-bold">核となる識別子を選択してください。</p>
              </div>
            </div>
            <div className="flex items-center justify-between mb-8">
              <p className="text-lg text-slate-500 dark:text-neutral-400 font-bold">候補: {activeProfile.constellationName}</p>
            ...
            <div className="flex flex-col gap-4">
              {activeProfile.constellationHistory[activeProfile.historyPointer]?.map((name) => (
                <button
                  key={name}
                ...
                >
                  ...
                  {activeProfile.constellationName === name && <CheckCircle2 size={24} />}
                </button>
              ))}
            </div>
          </section>
          ...
          <section className="space-y-12">
            <div className="flex items-start space-x-5">
              <div className="w-12 h-12 rounded-[1rem] flex items-center justify-center font-black text-lg shadow-lg shrink-0 bg-[var(--accent)] text-white">2</div>
              <div className="pt-1">
                <h2 className="text-xl font-black text-[var(--text)] tracking-tight leading-none mb-2 uppercase">プロフィールのタイプ</h2>
                <p className="text-sm text-slate-400 font-bold">用途に合わせてタイプを選択してください。</p>
              </div>
            </div>
          ...
          <section className="space-y-12">
            <div className="flex items-start space-x-5">
              <div className="w-12 h-12 rounded-[1rem] flex items-center justify-center font-black text-lg shadow-lg shrink-0 bg-[var(--accent)] text-white">3</div>
              <div className="pt-1">
                <h2 className="text-xl font-black text-[var(--text)] tracking-tight leading-none mb-2 uppercase">プロフィールの目的</h2>
                <p className="text-sm text-slate-400 font-bold">目的を選択すると、最適なスロットが自動的に構成されます。</p>
              </div>
            </div>
          ...
          <section className="space-y-12">
            <div className="flex items-start space-x-5">
              <div className="w-12 h-12 rounded-[1rem] flex items-center justify-center font-black text-lg shadow-lg shrink-0 bg-[var(--accent)] text-white">4</div>
              <div className="pt-1">
                <h2 className="text-xl font-black text-[var(--text)] tracking-tight leading-none mb-2 uppercase">スロット設定</h2>
                <p className="text-sm text-slate-400 font-bold">公開したい情報の種類を微調整できます。</p>
              </div>
            </div>
          ...
          <section className="space-y-12">
            <div className="flex items-start space-x-5">
              <div className="w-12 h-12 rounded-[1rem] flex items-center justify-center font-black text-lg shadow-lg shrink-0 bg-[var(--accent)] text-white">5</div>
              <div className="pt-1">
                <h2 className="text-xl font-black text-[var(--text)] tracking-tight leading-none mb-2 uppercase">データセット選択</h2>
                <p className="text-sm text-slate-400 font-bold">スロットに流し込むデータの塊を選択してください。</p>
              </div>
            </div>
