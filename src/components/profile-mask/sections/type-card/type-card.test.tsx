...
describe('TypeCard', () => {
  ...
  it('名前と説明文が正しく表示されること', () => {
    render(<TypeCard type={mockType} isSelected={false} onSelect={mockOnSelect} />);
  ...
  it('isSelectedがtrueの時に選択状態のデザインが適用されること', () => {
    render(<TypeCard type={mockType} isSelected={true} onSelect={mockOnSelect} />);

    expect(screen.getByRole('button')).toHaveAttribute('aria-pressed', 'true');
  });

  it('アクセシビリティ違反がないこと', async () => {
    const { container } = render(<TypeCard type={mockType} isSelected={false} onSelect={mockOnSelect} />);
