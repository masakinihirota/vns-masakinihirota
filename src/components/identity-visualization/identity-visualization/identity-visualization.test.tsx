...
describe('IdentityVisualization', () => {
  const defaultProps = {
    ...
    profileRefs: mockProfileRefs as unknown as React.MutableRefObject<Record<string, HTMLButtonElement | null>>,
  ...
  it('正しくレンダリングされる', () => {
    ...
    expect(screen.getAllByText(IDENTITY_CONFIG.account.name).length).toBeGreaterThan(0);
    expect(screen.getAllByText(IDENTITY_CONFIG.ghost.name).length).toBeGreaterThan(0);
  });
  ...
  it('プロフィールのクリックで onProfileSelect が呼ばれる', () => {
    ...
    maskButton.click();
    expect(defaultProps.onProfileSelect).toHaveBeenCalledWith(IDENTITY_CONFIG.masks[0].id);
  });
});
