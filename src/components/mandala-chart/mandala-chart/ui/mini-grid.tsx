...
export const MiniGrid: React.FC<MiniGridProps> = ({
  ...
  const getCellStyles = (gIdx: number, cIdx: number) => {
    let base = "relative group flex items-center justify-center w-full h-full p-1 cursor-pointer transition-all duration-200 border rounded-sm outline-none focus:ring-2 focus:ring-blue-500 focus:z-10 ";

    if (isFocusView) {
      base += "text-[18px] md:text-xl font-medium leading-tight h-24 md:h-32 ";
    } else {
      base += "text-[9px] md:text-[10px] lg:text-xs leading-none tracking-tight ";
    }
    ...
    if (cIdx === 4) {
      return base + "bg-blue-100 dark:bg-blue-900/40 text-blue-900 dark:text-blue-100 font-bold border-blue-200 dark:border-blue-800";
    }
    if (gIdx === 4) {
      return base + "bg-blue-50 dark:bg-blue-950/30 text-slate-800 dark:text-slate-200 border-blue-100 dark:border-blue-900";
    }
    return base + "bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800";
  };
    ...
    >
      {grids[gridIdx].map((cellText, cellIdx) => (
        ...
        >
          ...
          {((gridIdx === 4 && cellIdx !== 4) || (gridIdx !== 4 && cellIdx === 4)) && (
