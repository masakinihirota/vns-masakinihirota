...
import {
  ...
  Heart,
  BookOpen,
  ChartBar as BarChart2, // Lucide v0.263+ uses ChartBar for BarChart2
  ChevronRight,
  ArrowLeft,
  ExternalLink,
  Briefcase,
...
const CreationUI = () => {
  ...
  // 自分自身が作った作品リスト (詳細データを含む)
  const [myWorks] = useState<Work[]>([
    {
      id: 1,
      title: "オアシス宣言：第1章",
      category: "漫画",
  ...
  const getTierDisplay = (tier: Evaluation["tier"]) => {
    switch (tier) {
      case "tier1":
        return "1";
      case "tier2":
          ...
          <MarkdownTable
            ...
            rows={myWorks.map((w) => ({
              cells: [
                ...
                { content: w.category },
                { content: w.date, className: "text-right font-mono text-xs" },
              ],
            }))}
            onAdd={() => {}}
            addLabel="作品を追加"
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans transition-colors duration-300">
      <main className="max-w-6xl mx-auto px-8 py-12 pb-24">
        {currentPage === "profile" ? <ProfileView /> : <PortfolioView />}
