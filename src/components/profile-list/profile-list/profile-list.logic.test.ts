...
describe("profile-list.logic", () => {
  describe("filterProfilesByRole", () => {
    it("指定した役割のプロフィールのみを返す", () => {
      const result = filterProfilesByRole({ profiles: mockProfiles, role: "エンジニア" });
    ...
    it("一致する役割がない場合は空配列を返す", () => {
      const result = filterProfilesByRole({ profiles: mockProfiles, role: "存在しない役割" });
  ...
  describe("filterProfilesByName", () => {
    it("名前に検索文字列を含むプロフィールを返す", () => {
      const result = filterProfilesByName({ profiles: mockProfiles, searchText: "田" });
      expect(result).toHaveLength(2);
      expect(result[0]?.name).toBe("山田太郎");
    ...
    it("検索文字列が空の場合は全てのプロフィールを返す", () => {
      const result = filterProfilesByName({ profiles: mockProfiles, searchText: "" });
      expect(result).toHaveLength(4);
    });

    it("一致する名前がない場合は空配列を返す", () => {
      const result = filterProfilesByName({ profiles: mockProfiles, searchText: "存在しない名前" });
      expect(result).toHaveLength(0);
    });
  });
  ...
  describe("sortProfilesByName", () => {
    it("名前を昇順でソートする", () => {
      const result = sortProfilesByName({ profiles: mockProfiles, order: "asc" });
    ...
    it("名前を降順でソートする", () => {
      const result = sortProfilesByName({ profiles: mockProfiles, order: "desc" });
