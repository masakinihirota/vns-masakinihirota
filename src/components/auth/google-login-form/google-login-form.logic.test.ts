...
describe("useGoogleLoginLogic", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("正常にGoogle認証を実行できる", async () => {
    const mockSignIn = vi.mocked(signIn.social);
    mockSignIn.mockResolvedValue({ error: null } as any);

    const { result } = renderHook(() => useGoogleLoginLogic());
  ...
  it("エラーが発生した場合、エラー情報を設定する", async () => {
    ...
    await waitFor(() => {
      ...
      expect(getOAuthErrorInfo).toHaveBeenCalledWith(mockError, "google");
    });
  });
