...
import { Favorite, RatingType, SortConfig, ThemeVars } from '../features/profile-dashboard.types';
  ...
  <section className={`rounded-2xl ${themeVars.card} border overflow-hidden`}>
    ...
    <div className="overflow-x-auto">
      <table className="w-full border-collapse">
        <thead className={`${themeVars.subText} text-[14pt] uppercase tracking-widest text-left`}>
          <tr className="border-b border-white/10">
            <SortHeader label="タイトル" sortKey="title" currentSort={sortConfig} onSort={onSort} extraClass="pl-12" />
            <SortHeader label="カテゴリ" sortKey="subCategory" currentSort={sortConfig} onSort={onSort} />
            <th className="p-6 w-72">ジャンル</th>
            <SortHeader label="評価" sortKey="rating" currentSort={sortConfig} onSort={onSort} center />
            <th className="p-6 w-16"></th>
          </tr>
        </thead>
        <tbody>
          {favorites.map(item => (
            <tr key={item.id} className={`border-b border-white/5 transition-colors ${themeVars.rowHover} group`}>
              <EditableCell value={item.title} onSave={(v) => onUpdateCell(item.id, 'title', v)} themeVars={themeVars} extraClass="pl-12" />
              <td className="p-6">
                <span className={`px-4 py-1 rounded-full text-[14pt] font-bold ${item.subCategory === 'Manga' ? 'bg-amber-500/10 text-amber-500 border border-amber-500/20' : 'bg-purple-500/10 text-purple-500 border border-purple-500/20'}`}>
                  {item.subCategory}
                </span>
              </td>
              <EditableCell value={item.genre} onSave={(v) => onUpdateCell(item.id, 'genre', v)} themeVars={themeVars} />
              <RatingCell value={item.rating} isLikeMode={isLikeMode} onChange={(v) => onRatingChange(item.id, v)} themeVars={themeVars} />
