  ...
  <section className={`rounded-2xl ${themeVars.card} border overflow-hidden`}>
    ...
    <div className="overflow-x-auto">
      <table className="w-full border-collapse">
        ...
        <tbody>
          {skills.map(s => (
            <tr key={s.id} className={`border-b border-white/5 transition-colors ${themeVars.rowHover} group`}>
              <td className="p-6 pl-12 font-bold text-2xl">{s.name}</td>
              <td className={`p-6 text-[16pt] ${themeVars.subText}`}>{s.category}</td>
              <td className={`p-6 text-right font-mono text-3xl font-black ${themeVars.accent}`}>{s.level}</td>
