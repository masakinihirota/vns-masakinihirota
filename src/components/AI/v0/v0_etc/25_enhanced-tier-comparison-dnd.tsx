...
export default function Component() {
	...
	const { myFiltered, userAFiltered } = useMemo(
		...
		[myList.works, userAList.works, filterWorks]
	)
	...
	return (
		<DragDropContext onDragEnd={onDragEnd}>
			<div className='max-w-4xl mx-auto p-4 bg-gradient-to-r from-purple-400 via-pink-500 to-red-500 rounded-xl shadow-2xl'>
				...
				{/* sortedWorksをuseMemoでトップレベルで計算し、renderListに渡す */}
				{(() => {
					const mySortedWorks = useMemo(
						() => sortWorks(myFiltered),
						[myFiltered, sortMode]
					)
					const userASortedWorks = useMemo(
						() => sortWorks(userAFiltered),
						[userAFiltered, sortMode]
					)
					return (
						<div className='flex space-x-4'>
							{renderList(myList, mySortedWorks, "red", "myList", sortMode)}
							{renderList(
								userAList,
								userASortedWorks,
								"blue",
