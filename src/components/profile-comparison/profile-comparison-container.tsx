...
export const ProfileComparisonContainer: React.FC = () => {
  ...
  const [userInteractions, setUserInteractions] = useState<Record<string | number, UserInteraction>>({});
  ...
  const [sort, setSort] = useState<SortConfig>({ key: "heat", direction: "desc" });
  ...
  // --- Handlers ---
  const handleToggleAction = (id: string | number, type: keyof UserInteraction) => {
    ...
    <div className="flex h-screen bg-slate-100 text-slate-900 font-sans overflow-hidden">
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-slate-50">
        ...
        <footer className="px-8 py-3 bg-white border-t border-slate-200 flex justify-between items-center text-[10px] font-black text-slate-400 uppercase tracking-widest">
          <div>Sorted by: {sort.key} ({sort.direction})</div>
