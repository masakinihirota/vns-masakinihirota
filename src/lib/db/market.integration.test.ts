...
describe("Market Integration (Drizzle)", () => {
  ...
  afterAll(async () => {
    ...
    if (itemId) {
      try {
        await database.delete(marketItems).where(eq(marketItems.id, itemId));
      } catch {}
    }
    if (nationId) {
      try {
        await database.delete(nations).where(eq(nations.id, nationId));
      } catch {}
    }
  ...
  it("should start a transaction", async () => {
    ...
    const tx = await database.query.marketTransactions.findFirst({
