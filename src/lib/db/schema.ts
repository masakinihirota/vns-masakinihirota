...
export const rootAccounts = pgTable(
  ...
  (table) => [
    ...
    pgPolicy("Users can update own root account", {
      as: "permissive",
      for: "update",
      to: ["public"],
      using: sql`(auth.uid() = auth_user_id)`,
    }),
    pgPolicy("Users can view own root account", {
      as: "permissive",
      for: "select",
      to: ["public"],
    }),
...
export const userProfiles = pgTable(
  ...
  (table) => [
    ...
    pgPolicy("Users can delete own profiles", {
      ...
      for: "delete",
      to: ["public"],
      using: sql`(root_account_id IN ( SELECT root_accounts.id
   FROM root_accounts
  WHERE (root_accounts.auth_user_id = auth.uid())))`,
    }),
    pgPolicy("Users can update own profiles", {
      as: "permissive",
      for: "update",
      to: ["public"],
    }),
    pgPolicy("Users can insert own profiles", {
      as: "permissive",
      for: "insert",
      to: ["public"],
    }),
    pgPolicy("Users can view own profiles", {
      as: "permissive",
      for: "select",
      to: ["public"],
    }),
...
export const businessCards = pgTable(
  ...
  WHERE ((user_profiles.id = business_cards.user_profile_id) AND (root_accounts.auth_user_id = auth.uid()))))`,
    ...
    pgPolicy("Owners can update their own business card", {
      as: "permissive",
      for: "update",
      to: ["public"],
    }),
    pgPolicy("Public can view published business cards", {
      as: "permissive",
      for: "select",
      to: ["public"],
    }),
    pgPolicy("Owners can view their own business card", {
      as: "permissive",
      for: "select",
      to: ["public"],
    }),
...
export const alliances = pgTable(
  ...
  (table) => [
    ...
    pgPolicy("Users can delete their own profiles' alliances", {
      ...
      for: "delete",
      to: ["public"],
      using: sql`((profile_a_id IN ( SELECT user_profiles.id
   FROM user_profiles
  ...
  WHERE (user_profiles.root_account_id IN ( SELECT root_accounts.id
    ...
    pgPolicy("Users can update their own profiles' alliances", {
      as: "permissive",
      for: "update",
      to: ["public"],
    }),
    pgPolicy("Users can initiate alliances for their own profiles", {
      as: "permissive",
      for: "insert",
      to: ["public"],
    }),
    pgPolicy("Users can view their own profiles' alliances", {
      as: "permissive",
      for: "select",
      to: ["public"],
    }),
...
export const works = pgTable(
  ...
  (table) => [
    ...
    pgPolicy("Users can view their own pending/private works", {
      as: "permissive",
      for: "select",
      to: ["public"],
      using: sql`(auth.uid() = owner_user_id)`,
    }),
    pgPolicy("Public works are viewable by everyone", {
      as: "permissive",
      for: "select",
      to: ["public"],
    }),
    pgPolicy("Users can update their own works", {
      as: "permissive",
      for: "update",
      to: ["public"],
    }),
    pgPolicy("Users can insert their own works", {
      as: "permissive",
      for: "insert",
      to: ["public"],
    }),
...
export const groups = pgTable(
  ...
  (table) => [
    ...
    pgPolicy("Leaders can delete groups", {
      ...
      for: "delete",
      to: ["public"],
      using: sql`(leader_id IN ( SELECT user_profiles.id
   FROM user_profiles
  WHERE (user_profiles.root_account_id IN ( SELECT root_accounts.id
           FROM root_accounts
          WHERE (root_accounts.auth_user_id = auth.uid())))))`,
    }),
    pgPolicy("Leaders can update groups", {
      as: "permissive",
      for: "update",
      to: ["public"],
    }),
    pgPolicy("Authenticated users can create groups", {
      as: "permissive",
      for: "insert",
      to: ["public"],
    }),
    pgPolicy("Public read access for groups", {
      as: "permissive",
      for: "select",
      to: ["public"],
    }),
...
export const nations = pgTable(
  ...
  (table) => [
    ...
    pgPolicy("Public read access for nations", {
      as: "permissive",
      for: "select",
      to: ["public"],
      using: sql`true`,
...
export const marketItems = pgTable(
  ...
  (table) => [
    ...
    pgPolicy("Public read access for market items", {
      as: "permissive",
      for: "select",
      to: ["public"],
      using: sql`true`,
...
export const nationEvents = pgTable(
  ...
  (table) => [
    ...
    pgPolicy("Public read access for events", {
      as: "permissive",
      for: "select",
      to: ["public"],
      using: sql`true`,
...
export const follows = pgTable(
  ...
  (table) => [
    ...
    pgPolicy("Users can delete follows for their own profiles", {
      ...
      for: "delete",
      to: ["public"],
      using: sql`(follower_profile_id IN ( SELECT user_profiles.id
   FROM user_profiles
  WHERE (user_profiles.root_account_id IN ( SELECT root_accounts.id
           FROM root_accounts
          WHERE (root_accounts.auth_user_id = auth.uid())))))`,
    }),
    pgPolicy("Users can update follows for their own profiles", {
      as: "permissive",
      for: "update",
      to: ["public"],
    }),
    pgPolicy("Users can create follows for their own profiles", {
      as: "permissive",
      for: "insert",
      to: ["public"],
    }),
    pgPolicy("Users can view their profiles' followers (excluding watches)", {
      as: "permissive",
      for: "select",
      to: ["public"],
    }),
    pgPolicy("Users can view their own profiles' follows and watches", {
      as: "permissive",
      for: "select",
      to: ["public"],
    }),
...
export const groupMembers = pgTable(
  ...
  (table) => [
    ...
    pgPolicy("Users can leave groups", {
      ...
      for: "delete",
      to: ["public"],
      using: sql`(user_profile_id IN ( SELECT user_profiles.id
   FROM user_profiles
  WHERE (user_profiles.root_account_id IN ( SELECT root_accounts.id
           FROM root_accounts
          WHERE (root_accounts.auth_user_id = auth.uid())))))`,
    ...
    pgPolicy("Leaders can remove members", {
      as: "permissive",
      for: "delete",
      to: ["public"],
    }),
    pgPolicy("Leaders can update members", {
      as: "permissive",
      for: "update",
      to: ["public"],
    }),
    pgPolicy("Leaders can add members", {
      as: "permissive",
      for: "insert",
      to: ["public"],
    }),
    pgPolicy("Public read access for group_members", {
      as: "permissive",
      for: "select",
      to: ["public"],
    }),
...
export const userWorkRatings = pgTable(
  ...
  (table) => [
    ...
    pgPolicy("Users can insert their own ratings", {
      as: "permissive",
      for: "insert",
      to: ["public"],
    }),
    pgPolicy("Users can view their own ratings", {
      as: "permissive",
      for: "select",
      to: ["public"],
    }),
...
export const userWorkEntries = pgTable(
  ...
  (table) => [
    ...
    pgPolicy("Users can delete their own entries", {
      as: "permissive",
      for: "delete",
      to: ["public"],
      using: sql`(auth.uid() = user_id)`,
    }),
    pgPolicy("Users can update their own entries", {
      as: "permissive",
      for: "update",
      to: ["public"],
    }),
    pgPolicy("Users can insert their own entries", {
      as: "permissive",
      for: "insert",
      to: ["public"],
    }),
    pgPolicy("Users can view their own entries", {
      as: "permissive",
      for: "select",
      to: ["public"],
    }),
...
export const pointTransactions = pgTable(
  ...
  (table) => [
    ...
    pgPolicy("Users can view own point transactions", {
      as: "permissive",
      for: "select",
      to: ["public"],
      using: sql`(root_account_id IN ( SELECT root_accounts.id FROM root_accounts WHERE root_accounts.auth_user_id = auth.uid() ))`,
    }),
  ]
);
