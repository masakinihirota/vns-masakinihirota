...
export function getDeviceId(): string {
  if (globalThis.window === undefined) return "server-side-dummy-id";

  let deviceId = globalThis.localStorage.getItem(STORAGE_KEY_DEVICE_ID);
  if (!deviceId) {
    deviceId = uuidv4();
    globalThis.localStorage.setItem(STORAGE_KEY_DEVICE_ID, deviceId);
  }
  return deviceId;
}
...
export async function getLocalUserProfiles(
  ...
  const stored = globalThis.localStorage.getItem(STORAGE_KEY_PROFILES);
  if (!stored) return [];
...
export async function createLocalUserProfile(
  ...
  if (globalThis.window === undefined) {
    throw new TypeError("Cannot create local profile on server side");
  }
  ...
  const newProfile: UserProfile = {
    ...
    purpose: data.purpose ?? undefined, // Deprecated but keep for compat
    role_type: data.role_type ?? "member",
    is_active: true,
    last_interacted_record_id: undefined,
    created_at: now,
    updated_at: now,
  ...
  globalThis.localStorage.setItem(STORAGE_KEY_PROFILES, JSON.stringify(updatedAll));
