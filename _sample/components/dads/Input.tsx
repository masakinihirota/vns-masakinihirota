...
export const Input = forwardRef<HTMLInputElement, InputProps>((properties, reference) => {
  const { className, readOnly, isError, blockSize = "lg", ...rest } = properties;
  ...
  return (
    <input
      ...
      data-size={blockSize}
      readOnly={properties["aria-disabled"] ? true : readOnly}
      ref={reference}
      {...rest}
    />
  );
