...
const getCellStyles = (bIndex: number, cIndex: number) => {
  const isMainCenter = bIndex === 4 && cIndex === 4;
  const isSubCenter = (bIndex === 4 && cIndex !== 4) || (bIndex !== 4 && cIndex === 4);
...
interface BlockProperties {
  ...
  blocks: string[][];
  readOnly: boolean;
  setFocusedBlock: (blockIndex: number) => void;
  handleCellChange: (blockIndex: number, cellIndex: number, value: string) => void;
}
      ...
      >
        {isFocused ? (
          <input
            ...
            value={content}
            onChange={(e) =>
              handleCellChange(blockIdx, cellIndex, e.target.value)
            }
            readOnly={readOnly}
          />
        ) : (
          ...
          >
            <span className="line-clamp-3 text-[8px] sm:text-[10px] md:text-[11px] leading-tight break-all">
              {content || (blockIdx === 4 && cellIndex === 4 ? "テーマ" : "")}
            </span>
          </button>
        )}
      </div>
...
export function MandalaChart({
  ...
  const sourceReference = useRef<"markdown" | "chart" | "external">("external");

  // Initial Sync
  useEffect(() => {
    if (initialData) {
      sourceReference.current = "external";
  ...
  useEffect(() => {
    if (sourceReference.current === "markdown") {
              ...
              {[0, 1, 2, 3, 4, 5, 6, 7, 8].map((index) => (
                ...
                >
                  <div
                    ...
                    ${index === 4 ? "bg-indigo-600 text-white" : "bg-slate-700 dark:bg-zinc-800 text-white"}
                  `}
                  >
                    {index === 4 ? "★" : (index < 4 ? index + 1 : index)}
                  </div>
                  <Block
                    blockIdx={index}
                    isFocused={false}
