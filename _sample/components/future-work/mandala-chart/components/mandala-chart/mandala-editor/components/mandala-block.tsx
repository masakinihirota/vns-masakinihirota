...
interface MandalaBlockProperties {
...
export const MandalaBlock: React.FC<MandalaBlockProperties> = ({
  block,
  isCenterBlock,
  onClear,
  onCellClick,
  ...
  return (
    <div
      className={`grid grid-cols-3 grid-rows-3 gap-1 md:gap-1.5 rounded-lg p-1 h-full w-full transition-all duration-300 relative group/block shadow-sm ring-1 ${isCenterBlock
    ...
    >
      ...
      {[0, 1, 2, 3, 4, 5, 6, 7, 8].map((cellIndex) => {
        const isCellCenter = cellIndex === 4;
        const cellPeripheralIndex = GRID_POSITIONS.indexOf(
          cellIndex as (typeof GRID_POSITIONS)[number]
        );
        const text = isCellCenter
          ? block.title
          : (cellPeripheralIndex === -1
            ? ""
            : block.items[cellPeripheralIndex]);
        const label = isCellCenter
          ? block.label
          : `${block.label}-${cellPeripheralIndex + 1}`;
          ...
          <MandalaCell
            ...
            onClick={() =>
              onCellClick?.(
                ...
                isCellCenter ? "title" : cellPeripheralIndex
              )
            }
          />
        );
