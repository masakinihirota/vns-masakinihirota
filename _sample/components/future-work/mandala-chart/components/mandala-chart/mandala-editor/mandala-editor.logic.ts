...
export function parseMarkdownToMandala(markdown: string): MandalaData {
  ...
  const data: {
    ...
    center: { label: "★", title: "", items: Array.from({ length: 8 }, () => "") },
  ...
  for (const line of lines) {
    ...
    } else if (trimmed.startsWith("- [") && // 箇条書きアイテム (- [1] アイテム)
      currentBlock) {
...
export function generateMarkdownFromMandala(data: MandalaData): string {
  ...
  for (const [index, item] of data.center.items.entries()) {
    md += `- [${index + 1}] ${item}\n`;
  }
  md += "\n";
...
export { LABELS, SKELETON_DATA, type Label, type MandalaBlock, type MandalaData } from "./mandala-editor.types";
