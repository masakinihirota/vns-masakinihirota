...
interface MandalaEditorProperties {
  markdown: string;
...
export const MandalaEditor: React.FC<MandalaEditorProperties> = ({
  markdown,
  setMarkdown,
  data,
  copied,
          ...
          >
            ...
            >
              {[0, 1, 2, 3, 4, 5, 6, 7, 8].map((blockIndex) => {
                const isCenterBlock = blockIndex === CENTER_BLOCK_INDEX;
                const peripheralIndex =
                  blockIndex < CENTER_BLOCK_INDEX ? blockIndex : blockIndex - 1;
                const blockData = isCenterBlock
                  ? data.center
                  : data.blocks[peripheralIndex];
