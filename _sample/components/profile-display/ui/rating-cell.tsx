...
interface RatingCellProperties {
...
export const RatingCell = ({
  ...
  onChange,
  themeVars,
}: RatingCellProperties) => {
  const [isOpen, setIsOpen] = useState(false);

  const isTierValue = (
    [RATING_TYPES.TIER1, RATING_TYPES.TIER2, RATING_TYPES.TIER3] as RatingType[]
  ).includes(value);
  ...
  const getStyle = () => {
    if (isLikeMode && isTierValue)
      return "text-indigo-400 font-black dark:text-indigo-300";
    if (value === RATING_TYPES.TIER1)
      return "text-indigo-400 font-black dark:text-indigo-300";
    if (value === RATING_TYPES.TIER2)
      return "text-emerald-500 font-bold dark:text-emerald-400";
  ...
  return (
    <td className="p-0 relative group/cell border-l border-white/10">
      ...
      {isOpen && (
        <div
          className={`absolute z-50 top-full right-0 w-full mt-2 rounded-xl border p-2 shadow-2xl ${themeVars.card} min-w-[280px]`}
        >
          {isLikeMode ? (
            <button
            ...
            <div className="grid gap-1">
              <button
                className="p-4 text-left hover:bg-white/10 font-bold rounded-lg"
                onClick={() => {
                  onChange(RATING_TYPES.TIER1);
                  setIsOpen(false);
                }}
              >
                Tier 1 (最高)
          ...
          <button
            className="w-full p-4 text-left hover:bg-white/10 rounded-lg text-[14pt]"
            onClick={() => {
              onChange(RATING_TYPES.NEUTRAL);
              setIsOpen(false);
            }}
          >
            普通 or 自分には合わなかった
          ...
          <button
            className="w-full p-4 text-left hover:bg-white/10 rounded-lg text-[14pt]"
            onClick={() => {
              onChange(RATING_TYPES.INTERESTED_NONE);
              setIsOpen(false);
            }}
          >
            興味無し
          ...
          <button
            className="w-full p-4 text-left hover:bg-white/10 rounded-lg text-[14pt] text-gray-500"
            onClick={() => {
              onChange(RATING_TYPES.UNRATED);
              setIsOpen(false);
            }}
          >
            未評価にする
          ...
          <div
            className="bg-black/20 p-2 mt-2 rounded text-center text-[12pt] cursor-pointer"
