...
const App = () => {
  ...
  const sectionReferences = useRef<{ [key: number]: HTMLDivElement | null }>({});
  ...
  const scrollToEditorLine = (lineIndex: number) => {
    if (!editorReference.current || lineIndex === undefined) return;
    const textarea = editorReference.current;
    const lines = markdown.split("\n");
    const lineStartPos =
  ...
  const handleEditorScroll = (e: React.UIEvent<HTMLTextAreaElement>) => {
    ...
    const currentLine =
      textarea.value.slice(0, Math.max(0, textarea.selectionStart)).split("\n").length - 1;
    const active = mandalaData.reduce((previous, current) => {
      return current.lineStart <= currentLine ? current : previous;
    }, mandalaData[0]);
    if (active && active.id !== activeSectionId) {
      setActiveSectionId(active.id);
      if (viewMode === "list") {
        sectionReferences.current[active.id]?.scrollIntoView({
          behavior: "smooth",
          block: "start",
        });
      }
  ...
  const renderGrid = (section: Section | undefined) => {
    ...
    const cells: (Item | null)[] = Array.from({ length: 9 }, () => null);
    cells[4] = {
      text: section.title,
      isTitle: true,
    ...
    for (const [index, item] of section.items.slice(0, 8).entries()) {
      const pos = index >= 4 ? index + 1 : index;
      cells[pos] = { ...item, subId: index + 1 };
    }
  ...
  const fullMatrix = useMemo(() => {
    const matrix: (Section | undefined)[] = Array.from({ length: 9 }, () => undefined);
    const mainSection = mandalaData.find((s) => s.type === "main");
    matrix[4] = mainSection;
    for (let index = 1; index <= 8; index++) {
      const pos = index > 4 ? index : index - 1;
      matrix[pos] = mandalaData.find((s) => s.type === "sub" && s.id === index);
    }
    return matrix;
  }, [mandalaData]);
              ...
              {[0, 1, 2, 3, 4, 5, 6, 7, 8].map((blockId) => {
                ...
                return (
                  <div
                    key={blockId}
                    ref={(element) => {
                      sectionReferences.current[blockId] = element;
                    }}
        ...
        <div className="flex space-x-8">
          <span className="text-slate-500 flex items-center">
            ...
            <span className="text-emerald-400 ml-1 font-mono">
              {Math.round(
                (mandalaData.reduce(
                  (accumulator, s) => accumulator + s.items.filter((index) => index.completed).length,
                  0
                ) /
                  Math.max(
                    1,
                    mandalaData.reduce((accumulator, s) => accumulator + s.items.length, 0)
                  )) *
                100
              )}
              %
