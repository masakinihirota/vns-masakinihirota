...
export function useWorkSearch() {
  ...
  const [databaseResults, setDatabaseResults] = useState<Work[]>(() =>
  ...
  useEffect(() => {
    setPage(1);
    setDisplayedResults(databaseResults.slice(0, ITEMS_PER_PAGE));
    setHasMore(databaseResults.length > ITEMS_PER_PAGE);
  }, [databaseResults]);
  ...
  const loadMore = useCallback(() => {
    ...
    const nextItems = databaseResults.slice(0, nextPage * ITEMS_PER_PAGE);

    setDisplayedResults(nextItems);
    setPage(nextPage);
    setHasMore(databaseResults.length > nextPage * ITEMS_PER_PAGE);
  }, [databaseResults, hasMore, page]);
  ...
  useEffect(() => {
    if (query.trim()) {
      ...
      setDatabaseResults(MOCK_DB_WORKS.filter((w) => w.category === category));
    }
  ...
  return {
    ...
    setCategory,
    isSearching,
    dbResults: databaseResults, // オリジナル結果（件数表示などに使用可）
