...
describe("Step1ResidencePC", () => {
  ...
  const defaultProps = {
    data: {} as any, // Cast data to any here
    onUpdate: mockUpdate,
  };
  ...
  it("toggles residence area selection (deselects when clicked again)", () => {
    ...
    render(<Step1ResidencePC {...properties} />);
  ...
  it("shows correct input type based on country", () => {
    ...
    rerender(
      <Step1ResidencePC
        {...defaultProps}
        data={{ activity_culture_code: "japanese", selectedCountry: "日本" } as any}
      />
    );
    ...
    rerender(
      <Step1ResidencePC
        {...defaultProps}
        data={{ activity_culture_code: "english", selectedCountry: "アメリカ" } as any}
      />
    );
