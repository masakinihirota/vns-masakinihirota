...
import { OnboardingStepProperties } from "../onboarding-pc.types";

import { DailyScheduleEditor } from "./daily-schedule-editor";


interface Step2HoursPCProperties extends OnboardingStepProperties {
 ...
 * @param root0.weekSchedulerProps
 * @param root0.holidayLabel
 */
export function Step2HoursPC({
  data,
  onUpdate,
  weekSchedulerProps,
  holidayLabel = "休日用 (Holiday)",
}: Step2HoursPCProperties) {
  ...
  const calculateTotal = (
    ...
    const start1 = timeToHours((data as any)[startKey] || "09:00");
    const end1 = timeToHours((data as any)[endKey] || "18:00");
    const total1 = end1 - start1;

    let total2 = 0;
    if (subStartKey && subEndKey && data[subStartKey] && data[subEndKey]) {
      const start2 = timeToHours((data as any)[subStartKey]);
      const end2 = timeToHours((data as any)[subEndKey]);
      total2 = end2 - start2;
    }
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      {data.week_schedule && (
        <div className="mb-8 p-6 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700">
          ...
          <WeekScheduler
            ...
            onChange={(value) => onUpdate({ week_schedule: value })}
            {...weekSchedulerProps}
          />
        </div>
      )}
      ...
      <div className="space-y-4">
        ...
        <div
          className={`p-4 rounded-lg border flex items-start gap-3 ${workTotalHours < 8
            ? "bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800"
            : "bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800"
            }`}
        >
          <AlertCircle
            className={`w-5 h-5 flex-shrink-0 mt-0.5 ${workTotalHours < 8
          ...
          <div>
            <p
              className={`text-sm font-semibold ${workTotalHours < 8
      ...
      <div className="space-y-4">
        ...
        <div
          className={`p-4 rounded-lg border flex items-start gap-3 ${holidayTotalHours < 8
            ? "bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800"
            : "bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800"
            }`}
        >
          <AlertCircle
            className={`w-5 h-5 flex-shrink-0 mt-0.5 ${holidayTotalHours < 8
              ? "text-amber-600 dark:text-amber-400"
          ...
          <div>
            <p
              className={`text-sm font-semibold ${holidayTotalHours < 8
                ? "text-amber-900 dark:text-amber-100"
