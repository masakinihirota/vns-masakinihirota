...
describe("StepBasicValuesPC", () => {
  ...
  it("should have no accessibility violations", async () => {
    const { container } = render(
      <StepBasicValuesPC data={emptyData as any} onUpdate={mockOnUpdate} />
    );
  ...
  it("renders all 10 questions", () => {
    render(<StepBasicValuesPC data={emptyData as any} onUpdate={mockOnUpdate} />);
  ...
  it("highlights selected options", () => {
    ...
    render(<StepBasicValuesPC data={presetData as any} onUpdate={mockOnUpdate} />);
    ...
    const selectedButton = yesButtons[0];
    expect(selectedButton.className).toContain("bg-teal-500");
  });

  it("handles multi-select for opinion_diversity", () => {
    const data = { basic_values: { opinion_diversity: ["new_perspective"] } };
    render(<StepBasicValuesPC data={data as any} onUpdate={mockOnUpdate} />);
