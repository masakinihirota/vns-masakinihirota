...
import { hoursToTime, timeToHours } from "@/lib/root-account-utils";

import { OnboardingPCData } from "../onboarding-pc.types";

interface DailyScheduleEditorProperties {
  label: string;
  startKey: string;
  endKey: string;
  subStartKey?: string;
  ...
  onUpdate: (data: Partial<OnboardingPCData>) => void;
  colorClass?: string;
  thumbColorClass?: string;
}

export const DailyScheduleEditor: React.FC<DailyScheduleEditorProperties> = ({
  label,
  startKey,
  endKey,
  subStartKey,
  ...
  const nextDayEndHour =
    timeToHours((data[endKey] as string) || "18:00") > 24
      ? timeToHours(data[endKey] as string) - 24
      : 0;

  const nextDayEndHour2 =
    subEndKey && timeToHours((data[subEndKey] as string) || "00:00") > 24
      ? timeToHours(data[subEndKey] as string) - 24
      : 0;

  const handleChange = (field: string, value: unknown) => {
    onUpdate({ [field]: value } as Partial<OnboardingPCData>);
  };
        ...
        <div>
          <div className="flex justify-between items-center mb-4">
            ...
            <span className="text-lg font-bold text-slate-700 dark:text-slate-300">
              {hoursToTime(timeToHours((data[startKey] as string) || "09:00"))} ～{" "}
              {timeToHours((data[endKey] as string) || "18:00") >= 24
                ? "24:00"
                : hoursToTime(timeToHours((data[endKey] as string) || "18:00"))}
            </span>
          </div>
          ...
          <Slider.Root
            ...
            value={[
              timeToHours((data[startKey] as string) || "09:00"),
              Math.min(timeToHours((data[endKey] as string) || "18:00"), 24),
            ]}
        ...
        {timeToHours((data[endKey] as string) || "18:00") >= 24 && (
          <div className="pl-4 border-l-2 border-emerald-200 dark:border-emerald-800 animate-in fade-in slide-in-from-top-2">
            ...
            <Slider.Root
              ...
              onValueChange={(value) => {
                handleChange(endKey, hoursToTime(24 + value[0]));
              }}
              min={0}
              max={24}
              step={0.5}
              ...
              {isSubCoreHourEnabled ? (
                <button
                  onClick={() => {
                    ...
                    handleChange(subStartKey, "");
                    handleChange(subEndKey, "");
                  }}
            ...
            {isSubCoreHourEnabled && (
              <div className="animate-in fade-in slide-in-from-top-2 space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-lg font-bold text-slate-700 dark:text-slate-300">
                    {hoursToTime(timeToHours((data[subStartKey] as string) || "21:00"))} ～{" "}
                    {timeToHours((data[subEndKey] as string) || "23:00") >= 24
                      ? "24:00"
                      : hoursToTime(timeToHours((data[subEndKey] as string) || "23:00"))}
                  </span>
                </div>
                <Slider.Root
                  className="relative flex items-center select-none touch-none w-full h-5"
                  value={[
                    timeToHours((data[subStartKey] as string) || "21:00"),
                    Math.min(timeToHours((data[subEndKey] as string) || "23:00"), 24),
                  ]}
                ...
                {timeToHours((data[subEndKey] as string) || "00:00") >= 24 && (
                  <div className="pl-4 border-l-2 border-emerald-200 dark:border-emerald-800 animate-in fade-in slide-in-from-top-2">
                    <div className="flex justify-between items-center mb-2">
                      <label className="text-xs font-medium text-slate-500 dark:text-slate-400">
                        翌日の終了時間 (第二活動時間)
                    ...
                    <Slider.Root
                      ...
                      onValueChange={(value) => {
                        handleChange(subEndKey, hoursToTime(24 + value[0]));
                      }}
                      min={0}
                      max={24}
                      step={0.5}
