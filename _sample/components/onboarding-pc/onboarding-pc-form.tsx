...
interface OnboardingPCFormProperties {
  userId: string;
  initialData?: {
...
export function OnboardingPCForm({
  userId,
  initialData,
}: OnboardingPCFormProperties) {
  const router = useRouter();
  const [currentStep, setCurrentStep] = useState(1);
  const [formData, setFormData] = useState<OnboardingPCData>({
    // Initial state matching fields
    activity_area_id: null as number | null, // Default or null
    ...
    display_id: `user-${userId.slice(0, 8)}`, // Mock display ID
  ...
  const handleUpdate = (data: Partial<OnboardingPCData>) => {
  ...
  const nextStep = () => {
    ...
    setCurrentStep((previous) => Math.min(previous + 1, 8));
  };
