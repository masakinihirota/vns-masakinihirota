...
interface Step5ValuesProperties {
...
export const Step5Values = ({
  ...
  const handleAddValue = (id: string) => {
    setAddedQuestionIds((previous) => [...previous, id]);
    setRemovedQuestionIds((previous) => previous.filter((rid) => rid !== id)); // Clear removal if re-added
  };

  const handleRemoveValue = (id: string) => {
    setRemovedQuestionIds((previous) => [...previous, id]);
    setAddedQuestionIds((previous) => previous.filter((aid) => aid !== id)); // Clear addition if removed
                ...
                <div className="flex items-center gap-1">
                  {[1, 2, 3].map((t) => (
                    <button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        toggleValueTier(q.id, t as 1 | 2 | 3);
                      }}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
          ...
          <div className="bg-white dark:bg-slate-900 border border-indigo-200 dark:border-indigo-800/50 rounded-xl shadow-sm p-4 overflow-hidden ring-1 ring-indigo-50 dark:ring-indigo-900/20">
            ...
            {registeredValues.length === 0 ? (
              ...
              <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-1">
                {registeredValues.map((q, index) =>
                  renderQuestionCard(q, index, "REGISTERED")
                )}
              </div>
            )}
          </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="space-y-4 min-h-[400px]">
            {paginatedCandidates.map((q, index) =>
              renderQuestionCard(q, index, "CANDIDATE")
            )}
          </div>
