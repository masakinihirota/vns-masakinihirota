 ...
 * ```
 */
export const Radio = forwardRef<HTMLInputElement, RadioProps>((properties, reference) => {
  const { children, isError, onClick, size = "sm", ...rest } = properties;
  ...
  const radio = (
    ...
    >
      <input
        ...
        ref={reference}
        type="radio"
        onClick={properties["aria-disabled"] ? handleDisabled : onClick}
        data-size={size}
