/**
 * デジタル庁デザインシステム（DADS） - Button コンポーネント
 * @see https://design.digital.go.jp/
 * @license MIT - Copyright (c) 2025 デジタル庁
 *
 * このファイルはAI実装時の参照用サンプルです。
 * 実際の開発では Shadcn/UI の Button コンポーネントに対し、
...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  (properties, reference) => {
    ...
    const classNames = `${buttonBaseStyle} ${buttonSizeStyle[size]} ${variant ? buttonVariantStyle[variant] : ""
      } ${className ?? ""}`;
    ...
    return (
      <button
        ...
        onClick={properties["aria-disabled"] ? handleDisabled : properties.onClick}
        {...rest}
        ref={reference}
      >
        {children}
