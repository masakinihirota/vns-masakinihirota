...
type BusinessCardEditorProperties = {
  profile: UserProfile;
...
export function BusinessCardEditor({
  ...
  const handleConfigChange = <K extends keyof BusinessCardConfig>(
    key: K,
    value: BusinessCardConfig[K]
  ) => {
    setCard((previous) => ({
      ...previous,
      display_config: {
        ...previous.display_config,
        [key]: value,
      },
    }));
  };

  const handleContentChange = <S extends keyof BusinessCard["content"]>(
    section: S,
    key: string,
    value: unknown // content keys are dynamic strings, sticking to unknown for type safety
  ) => {
    setCard((previous) => ({
      ...previous,
      content: {
            ...
            <div className="flex items-center justify-between space-x-2 border p-4 rounded-lg bg-slate-50">
              ...
              <Switch
                ...
                onCheckedChange={(checked) =>
                  setCard((previous) => ({ ...previous, is_published: checked }))
                }
              />
            </div>
