...
export const normalizeRating = (
  ...
  const oldObject = rating as { status: RatingStatus; value: RatingValue };
  if (["Tier1", "Tier2", "Tier3"].includes(oldObject.value)) {
    return {
      status: oldObject.status,
      isLiked: true,
      tier: oldObject.value as "Tier1" | "Tier2" | "Tier3",
      otherValue: null,
    };
  }
  return {
...
export const getRatingsFromStorage = (
  ...
  try {
    ...
    console.error("Failed to parse ratings from storage", error);
    return {};
  }
};
...
export const exportRatings = (
  ...
  for (const title of Object.keys(ratings)) {
  ...
  const output = rated
    .map((index) => {
      ...
      const r = normalizeRating(index.rating);
      const ratingString = r.isLiked
        ? `${r.status}:${r.tier}`
        : `${r.status}:${r.otherValue || "未評価"}`;
      return `[${ratingString}][${index.title}]`;
    })
    .filter((s) => s !== "")
    .join("\n");
