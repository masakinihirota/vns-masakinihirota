...
export const WorkContinuousRatingContainer: React.FC = () => {
  ...
  const handleRate = useCallback(
    (value: RatingValue) => {
      ...
      const previousRating = normalizeRating(ratings[currentTitle]); // Get current state for memory
      ...
      if (["Tier1", "Tier2", "Tier3"].includes(value)) {
        ...
        newRating = {
          status: statusToSave,
          isLiked: false,
          tier: previousRating.tier, // Keep memory
          otherValue: value,
        };
      }
  ...
  const handlePrevious = useCallback(() => {
    if (currentIndex > 0) {
      ...
      // 戻った時のステータス復元
      const previousTitle = sessionList[currentIndex - 1];
      const previousRating = ratings[previousTitle];
      if (previousRating) {
        const norm = normalizeRating(previousRating);
  ...
  return (
    <WorkContinuousRating
      ...
      onToggleRatedItems={() => setShowRatedItems((previous) => !previous)}
    />
  );
};
