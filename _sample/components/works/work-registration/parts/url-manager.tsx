...
interface UrlManagerProperties {
  control: Control<WorkFormValues>;
}

export const UrlManager: React.FC<UrlManagerProperties> = ({ control }) => {
  return (
        ...
        <FormField
          ...
          render={({
            ...
            <div className="space-y-4">
              {field.value.map(
                (urlItem: { value?: string; type?: string }, index: number) => (
                  <div
                    key={index}
                        ...
                        <Button
                          ...
                          onClick={() => {
                            const newUrls = field.value.filter(
                              (_: unknown, index_: number) => index_ !== index
                            );
                            field.onChange(newUrls);
                          }}
