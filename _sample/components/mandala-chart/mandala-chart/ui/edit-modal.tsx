...
interface EditModalProperties {
  ...
  value: string;
  onUpdate: (gridIndex: number, cellIndex: number, value: string) => void;
  onClose: () => void;
}
 ...
 * セル編集用モーダルコンポーネント
 * @param root0
 * @param root0.editingCell
 * @param root0.value
 * @param root0.onUpdate
 * @param root0.onClose
 */
export const EditModal: React.FC<EditModalProperties> = ({
  editingCell,
  value,
  onUpdate,
  onClose,
    ...
    >
      ...
      >
        <div className="flex justify-between items-start mb-4 shrink-0">
          <div>
            <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">
              {GRID_POSITIONS[editingCell.gridIdx]} - Position{" "}
        ...
        <textarea
          ...
          value={value}
          onChange={(e) =>
            onUpdate(editingCell.gridIdx, editingCell.cellIdx, e.target.value)
          }
