...
import { Evaluation, ProfileEvaluations } from "./profile-evaluations";
...
describe("ProfileEvaluations", () => {
  ...
  it("初期表示（評価は好き一択モード）でTier 1, 2, 3の作品のみが表示され、Tier列が表示されないこと", async () => {
    ...
    const simpleButton = screen.getByText("評価は好き一択");
    expect(simpleButton).toHaveClass("bg-slate-900");
  ...
  it("モードを切り替えると、全ての作品が表示され、Tier列が表示されること", async () => {
    ...
    const tieredButton = screen.getByText("評価は絶対相対評価 Tier方式");
    fireEvent.click(tieredButton);
