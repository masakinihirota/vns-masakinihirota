...
const ProfileView = (properties: ProfileEditProps) => {
  const {
    profile,
    myWorks,
    evaluations,
...
const PortfolioView = (properties: ProfileEditProps) => {
  const { profile, myWorks, onSetPage } = properties;
  return (
    <div className="py-8">
      <button
...
export const ProfileEdit = (properties: ProfileEditProps) => {
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans transition-colors duration-300">
      ...
      {(properties.mode === "create" || properties.mode === "edit") && properties.onSave && (
        <div className="fixed bottom-8 right-8 z-50">
          ...
          >
            <Save size={20} />
            {properties.mode === "create" ? "プロフィールを作成" : "変更を保存"}
          </button>
        </div>
      )}
    </div>
