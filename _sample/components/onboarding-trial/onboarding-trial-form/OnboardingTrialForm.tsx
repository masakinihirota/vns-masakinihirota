...
export type TrialFormData = Omit<TrialRootAccount, "display_name" | "created_at"> & {
  is_trial: boolean;
};
...
export function OnboardingTrialForm() {
  ...
  const [formData, setFormData] = useState<TrialFormData>({
    ...
    display_id: `trial-${Math.random().toString(36).slice(2, 10)}`,
    nativeLanguages: [],
  ...
  const nextStep = () => {
    setCurrentStep((previous) => Math.min(previous + 1, 6));
  };
  const previousStep = () => setCurrentStep((previous) => Math.max(previous - 1, 1));

  const handleSubmit = async () => {
    setIsSubmitting(true);
    // 📌 境界防御層: 非同期の登録処理およびストレージ操作のエラーを捕捉し、ユーザーに通知する
    try {
  ...
  const renderStep = () => {
    // 既存コンポーネント（OnboardingPCData を期待）との型不整合を解消するため、
    // ここでは Partial<TrialFormData> を any としてから適切な型へキャスト、
    // またはコンポーネント側を柔軟にする必要がありますが、
    // 今回は OnboardingTrialForm 内での整合性を優先します。
    switch (currentStep) {
      case 1: {
        return (
          <Step1ResidencePC data={formData as typeof formData as any} onUpdate={handleUpdate as typeof handleUpdate as any} />
        );
      }
      case 2: {
        return <Step1ResidencePC data={formData as typeof formData as any} onUpdate={handleUpdate as typeof handleUpdate as any} />;
      }
      case 3: {
        return (
          <Step2HoursPC
            data={formData as typeof formData as any}
            onUpdate={handleUpdate as typeof handleUpdate as any}
            weekSchedulerProps={{
              labels: { MATCH: "自由行動時間" },
      ...
      case 4: {
        return <Step3IdentityTrial data={formData as any} onUpdate={handleUpdate} />;
      }
      case 5: {
        return <Step4LanguagePC data={formData as typeof formData as any} onUpdate={handleUpdate as typeof handleUpdate as any} />;
      }
      case 6: {
        return (
