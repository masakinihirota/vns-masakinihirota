...
import {
  ...
  MANDALA_TEMPLATES,
  MY_PROFILES,
} from "./skill-comparison.sample-data";
import { SkillSelector } from "./skill-selector";

/**
 * スキル比較マンダラチャートのメインコンテナ・コンポーネント
 *
 * 仕様書に定義されたデータフローとアルゴリズム（ソート、カテゴリ統合）を実装します。
 * 副作用（カテゴリ切り替え時のスキル追従）やインタラクション状態を管理します。
 */
export const MandalaContainer: React.FC = () => {
  ...
  const followed = useMemo(
    () => sortedCandidatesList.filter((c) => interactions[c.id]?.followed),
    [sortedCandidatesList, interactions]
  );
  const watched = useMemo(
    () =>
      sortedCandidatesList.filter(
  ...
  const handleCategoryChange = (newCategory: string) => {
    ...
    // 新しいカテゴリに対応するスキルが存在するか確認
    const availableSkills = Object.keys(MANDALA_TEMPLATES).filter(
      (s) => MANDALA_TEMPLATES[s].category === newCategory
    );

    // 現在のスキルが新カテゴリで利用可能でない場合、最初のスキルに変更
  ...
  return (
    <div className="flex h-screen bg-slate-50 text-slate-900 font-sans overflow-hidden">
      ...
      <aside
        className="w-80 bg-white border-r border-slate-200 flex flex-col shrink-0 z-30 shadow-sm"
        aria-label="マイアイデンティティ選択"
      >
              ...
              <div className="flex items-center space-x-4">
                ...
                {currentTarget && (
                  <div className="flex items-center space-x-3">
                    <div className="px-3 py-1 bg-indigo-50 text-indigo-600 text-[10px] font-black rounded border border-indigo-100 uppercase">
                      Mastery Active
                    </div>
            ...
            <div className="grid grid-cols-3 gap-8 p-10 bg-white rounded-3xl border border-slate-200 shadow-sm">
              <div className="flex items-start space-x-5">
                ...
                <div className="flex flex-col pt-1">
                  <span className="text-sm font-black uppercase tracking-tight mb-1">
                    Advice Mode
                  </span>
                  <p className="text-sm text-slate-400 leading-relaxed">
                    あなたが習得済み、相手に教えることができる分野です。
                  </p>
                </div>
              </div>
            ...
            <div className="flex flex-col items-end">
              ...
              >
                {sortOrder === "desc" ? (
                  ...
                  <LucideIcons.SortAsc size={18} />
                )}
              </button>
            </div>
            <span className="bg-slate-900 text-white text-xs px-3 py-1 rounded-lg font-black tabular-nums">
              {queue.length}
            </span>
          </div>
        </div>
