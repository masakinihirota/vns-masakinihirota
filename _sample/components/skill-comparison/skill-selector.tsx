...
interface SkillSelectorProperties {
...
/**
 * カテゴリとスキルのセレクター・コンポーネント
 *
 * 仕様書の「カテゴリとスキルの整合性」管理は Container 側で行われることを想定しています。
 * アクセシビリティ指針に基づき、文字サイズは 1rem (text-base) 以上を確保しています。
 * @param root0
 * @param root0.selectedCategory
 * @param root0.onSelectCategory
 * @param root0.focusedSkill
 * @param root0.onSelectSkill
 */
export const SkillSelector: React.FC<SkillSelectorProperties> = ({
  selectedCategory,
  onSelectCategory,
  focusedSkill,
  onSelectSkill,
        ...
        <div className="flex space-x-3">
          {CATEGORIES.map((cat) => {
            ...
            return (
              ...
              >
                {React.createElement((Icon as unknown) as any, {
                  size: 18,
