...
describe("WorkList UI", () => {
  ...
  it("評価モードの切り替えボタンが機能する", () => {
    render(<WorkList {...defaultProps} />);
    const tierModeButton = screen.getByTitle("Tier方式");
    const likeModeButton = screen.getByTitle("好き方式");

    fireEvent.click(likeModeButton);
    expect(defaultProps.onRatingModeToggle).toHaveBeenCalledWith("like");

    fireEvent.click(tierModeButton);
  ...
  it("作品選択時に評価パネルが表示される", () => {
    ...
    // Tierボタンが表示されているか（複数存在する可能性があるため getAllByText を使用）
