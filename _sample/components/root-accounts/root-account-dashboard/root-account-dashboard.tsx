...
import {
  AreaHistoryEntry,
  calculateAreaHistory,
  calculateGenerationHistory,
...
interface RootAccountDashboardProperties {
  data: RootAccount;
}
 ...
 * @param root0.data
 */
export function RootAccountDashboard({ data }: RootAccountDashboardProperties) {
  ...
  const [selectedArea, setSelectedArea] = useState<number | undefined>(
    normalizedData.activity_area_id ?? undefined
  );
  const [originalArea, setOriginalArea] = useState<number | undefined>(
    normalizedData.activity_area_id ?? undefined
  );
  const [areaHistory, setAreaHistory] = useState<readonly AreaHistoryEntry[]>(
    []
  );
  ...
  const handleSaveArea = (
    ...
    saveAreaSetting(targetArea, _targetStart, _targetEnd)
      .then(() => {
        setAreaHistory((previous) =>
          calculateAreaHistory(previous, originalArea, targetArea)
        );
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white dark:bg-slate-900 shadow rounded-lg p-6">
            <div className="flex justify-between items-center mb-4">
              <div>
                <h4 className="text-lg leading-6 font-medium text-slate-900 dark:text-slate-50">
                  マイ・プロフィール (My Profiles)
                </h4>
                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
          ...
          <div className="bg-white dark:bg-slate-900 shadow rounded-lg p-6">
            <div className="flex justify-between items-center mb-4">
              ...
              <button
                ...
                disabled={isLoading}
                aria-label={isEditingCountries ? "管理対象国を保存" : "管理対象国を編集"}
              ...
              return (
                <div
                  ...
                  onClick={() => {
                    ...
                    const def =
                      AREA_DEFINITIONS[
                      areaNumber as keyof typeof AREA_DEFINITIONS
                      ];
                    ...
                    void handleSaveArea(areaNumber, newStart, newEnd);
                  }}
                  className={`
                ...
                >
                  <div
                    ...
                    ${selectedArea === areaNumber ? "opacity-100" : "opacity-75"}
                  `}
                  >
                    {imageErrors.has(areaNumber) ? (
                      ...
                      <Image
                        src={`/world/area${areaNumber}.svg`}
                        alt={def.name}
                        fill
                        className="object-cover"
                        priority={areaNumber === 1}
                        onError={() => {
                          setImageErrors((previous) => new Set(previous).add(areaNumber));
                        }}
                      />
                    )}
                  </div>
        ...
        <div className="bg-white dark:bg-slate-900 shadow rounded-lg p-6">
          <div className="flex justify-between items-center mb-6">
            ...
            <button
              ...
              disabled={isLoading}
              aria-label={isEditingCoreHours ? "活動時間を保存" : "活動時間を編集"}
            ...
            <div>
              ...
              {isEditingCoreHours ? (
                (() => {
                  ...
                  const startLocalValue = toLocal(
                    timeToHours(formData.core_hours_start)
                  );
                  let endLocalValue = toLocal(
                    timeToHours(formData.core_hours_end)
                  );

                  if (endLocalValue < startLocalValue) endLocalValue += 24;

                  return (
                    <>
                      <Slider.Root
                        className="relative flex items-center select-none touch-none w-full h-5"
                        value={[startLocalValue, Math.min(endLocalValue, 24)]}
                  ...
                  <div className="flex items-center gap-2">
                    {isEditing && (
                      <button
                        ...
                        disabled={isLoading}
                        aria-label="ユーザー属性の編集をキャンセル"
                        className="px-3 py-1.5 border border-slate-300 dark:border-slate-700 rounded-md shadow-sm text-xs font-medium text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 disabled:opacity-50 transition-colors"
                      >
                        キャンセル
                      </button>
                    )}
                    <button
                      ...
                      disabled={isLoading}
                      aria-label={isEditing ? "ユーザー属性を保存" : "ユーザー属性を編集"}
