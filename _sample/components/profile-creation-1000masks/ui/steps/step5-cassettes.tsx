...
interface Step5CassettesProperties {
...
const CassettePanel: React.FC<{
  ...
  return (
    <div
      className={`p-10 rounded-[2.5rem] shadow-xl hover:shadow-2xl transition-all border-l-8 text-left ${s.bg} ${s.border} ${s.borderL}`}
    >
      <div
        className={`flex items-center space-x-4 mb-8 text-left ${s.textColor}`}
      >
        ...
        <div className="flex flex-col text-left">
          ...
          <span
            className={`text-[12px] font-bold uppercase mt-1 ${colorTheme === "slate" ? "text-slate-500" : (colorTheme === "rose" ? "text-rose-400" : "text-slate-400 dark:text-slate-500")}`}
          >
            {slotLabel}
          </span>
        </div>
...
export const Step5Cassettes: React.FC<Step5CassettesProperties> = ({
  activeProfile,
  onUpdateDraft,
  onOpenModal,
}) => {
  const [expandedValueCategory, setExpandedValueCategory] = useState<
    string | null
  >(null);
  ...
  return (
    <section className="transition-colors duration-300">
      <StepHeader
        num="5"
        title="カセットの選択と装着"
        subtitle="選んだスロットに、どのカセット（データ）をセットするか決定します。"
      />
      {activeProfile.selectedSlots.length === 0 ? (
        <div className="py-20 text-center bg-slate-50 dark:bg-white/5 rounded-[3rem] border-2 border-dashed border-slate-200 dark:border-white/10 shadow-inner group">
          <LayoutGrid
            size={64}
            className="mx-auto text-slate-300 dark:text-slate-700 mb-6 opacity-30 group-hover:scale-110 transition-transform duration-500"
          />
          <p className="text-slate-400 dark:text-slate-500 font-black text-xl">
            スロットを選択すると、対応するパネルが表示されます。
          </p>
        </div>
      ) : (
        <div className="flex flex-col gap-12 text-left">
          {activeProfile.selectedSlots.includes("works") && (
            ...
            >
              <TabSelector
                ...
                onCreateNew={() =>
                  onOpenModal("info", "作品用カセット作成画面へ移動します。")
                }
              />
            </CassettePanel>
          )}
          {activeProfile.selectedSlots.includes("skills") && (
            ...
            >
              <TabSelector
                ...
                onCreateNew={() =>
                  onOpenModal("info", "スキル用カセット作成画面へ移動します。")
                }
              />
            </CassettePanel>
          )}
          {activeProfile.selectedSlots.includes("partner_meta") && (
            <CassettePanel
              title="パートナーカセット"
              slotLabel="Slot: Partner"
              icon={<Heart size={36} />}
              colorTheme="rose"
            >
              <div className="space-y-6 text-left">
                <p className="text-lg font-bold text-rose-500 dark:text-rose-400 leading-relaxed italic">
                  全プロフィール内で1つだけの限定カセットです。
                </p>
                <div className="space-y-2">
                  <label className="text-lg font-black text-rose-600 dark:text-rose-400 tracking-widest ml-2">
                    属性（男女など）
                  </label>
                  <input
                    type="text"
          ...
          {activeProfile.selectedSlots.includes("politics") && (
            <CassettePanel
              title="政治カセット"
              slotLabel="Slot: Politics"
              icon={<Flag size={36} />}
              colorTheme="slate"
            >
              <div className="space-y-6 text-left">
                <div className="space-y-2">
                  <label className="text-lg font-black text-blue-400 tracking-widest ml-2">
                    応援政党
                  </label>
                  <input
                    type="text"
        ...
        <div className="p-10 bg-white dark:bg-white/5 border border-slate-100 dark:border-white/10 rounded-[4rem] shadow-xl mt-12 border-l-[12px] border-l-indigo-500 transition-all text-left">
          <div className="flex items-center justify-between mb-12 text-slate-800 dark:text-neutral-100">
            <div className="flex items-center space-x-5">
              ...
              />
              <span className="font-black text-xl tracking-tighter uppercase leading-none pt-2">
                価値観カセット装着
              </span>
            </div>
          </div>
          <div className="flex flex-col gap-4">
            {VALUE_CATEGORIES.map((cat) => {
              ...
              return (
                <div
                  key={cat.id}
                  className={`border-2 rounded-[2.5rem] overflow-hidden transition-all hover:shadow-xl ${isSelected
                ...
                >
                  <button
                    onClick={() =>
                      setExpandedValueCategory(isExpanded ? null : cat.id)
                    }
                  ...
                  >
                    <div className="flex items-center space-x-6">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          const newVals = isSelected
                            ? activeProfile.selectedValues.filter(
                              (v) => v !== cat.id
                            )
                      ...
                      <div className="flex flex-col">
                        ...
                        <span
                          className={`text-lg font-black leading-none ${isSelected ? "text-indigo-600 dark:text-indigo-400" : "text-slate-700 dark:text-slate-400"}`}
                        >
                          {cat.name}{" "}
                          {cat.isRoot && (
                            <span className="text-xs ml-2 bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400 px-2 py-1 rounded-full">
                              必須
                            </span>
                          )}
                        </span>
                    ...
                    <div className="flex flex-col items-end shrink-0 ml-4">
                      <span className="text-lg font-black text-slate-400 dark:text-slate-600 mb-2 leading-none">
                      ...
                      <div className="w-24 bg-slate-100 dark:bg-white/10 h-2.5 rounded-full overflow-hidden shadow-inner uppercase">
                        <div
                          className="h-full bg-indigo-500 dark:bg-indigo-600 transition-all duration-1000"
                          style={{
