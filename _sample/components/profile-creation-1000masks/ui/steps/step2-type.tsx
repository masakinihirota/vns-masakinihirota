...
import { PROFILE_TYPES } from "../../profile-creation-1000masks.constants";
import {
  Profile,
  ProfileTypeId,
...
interface Step2TypeProperties {
  activeProfile: Profile;
  onUpdateDraft: (updatedFields: Partial<Profile>) => void;
  error?: string;
}

export const Step2Type: React.FC<Step2TypeProperties> = ({
  activeProfile,
  onUpdateDraft,
  error,
}) => {
        ...
        {PROFILE_TYPES.map((type) => {
          ...
          return (
            <button
              key={type.id}
              onClick={() =>
                onUpdateDraft({ selectedTypeId: type.id as ProfileTypeId })
              }
            ...
            >
              <div
                className={`p-5 rounded-2xl mr-8 transition-colors shrink-0 ${isSelected
              ...
              <div className="flex-1 text-left">
                <div className="flex items-center justify-between mb-2 text-left">
                  <span
                    className={`text-lg font-black ${isSelected ? "text-white" : "text-slate-800 dark:text-neutral-200"}`}
                  >
                    {type.label}
                  </span>
                  {isSelected && (
                ...
                <p
                  className={`text-lg font-bold leading-relaxed ${isSelected ? "text-blue-50/80 dark:text-blue-200/70" : "text-slate-400 dark:text-slate-500"}`}
                >
                  {type.description}
                </p>
              </div>
