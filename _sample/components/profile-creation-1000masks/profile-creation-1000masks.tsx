...
interface ProfileCreationProperties {
  adapter?: ProfileStorageAdapter;
  isTrial?: boolean;
}

export const ProfileCreation1000Masks: React.FC<ProfileCreationProperties> = ({
  adapter,
  isTrial,
}) => {
  return (
...
const ProfileCreationContent: React.FC<ProfileCreationProperties> = ({
  adapter,
  isTrial,
}) => {
  ...
  const fromTrial = searchParameters.get("from") === "trial";
              ...
              <>
                ...
                <Step5Cassettes
                  ...
                  onOpenModal={(type: ModalState["type"], message: string) =>
                    setModal({ isOpen: true, type, targetId: undefined, message })
                  }
                />
              </>
            )}
