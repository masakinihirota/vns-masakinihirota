...
import { importTrialData } from "./trial-migration-modal.logic";
...
export function TrialMigrationModal() {
  ...
  // 意味のあるデータがある場合のみ初期化 (遅延初期化で setState in effect を防止)
  const [data, setData] = useState<VNSTrialData | null>(() => {
  ...
  // データが存在する場合、ダイアログを開く
  useEffect(() => {
    // SSR hydration: setIsOpen after render (expected pattern)
    if (data) {
