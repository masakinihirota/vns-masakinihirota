...
export type MandalaChartProps = {
  readonly charts: readonly MandalaChartType[];
  readonly activeIndex: number;
  readonly viewMode: "full" | "focus";
  readonly focusIndex: number;
  readonly editingCell: { readonly gridIdx: number; readonly cellIdx: number } | null;
  readonly isSidebarOpen: boolean;
  readonly saveStatus: "saved" | "saving";
  ...
  readonly onDeleteChart: (id: string, e: React.MouseEvent) => void;
  readonly onUpdateCell: (gridIndex: number, cellIndex: number, value: string) => void;
  readonly onChangeActiveIndex: (index: number) => void;
  readonly onChangeViewMode: (mode: "full" | "focus") => void;
  readonly onChangeFocusIndex: (index: number) => void;
  readonly onSetEditingCell: (
    cell: { readonly gridIdx: number; readonly cellIdx: number } | null
  ) => void;
  readonly onToggleSidebar: (isOpen: boolean) => void;
...
const getCellStyles = (
  ...
  base += isFocusView ? "text-[18px] md:text-xl font-medium leading-tight " : "text-[9px] md:text-[10px] lg:text-xs leading-none tracking-tight ";

  if (gridIndex === 4 && cellIndex === 4) {
    return base + "bg-blue-600 text-white font-bold border-blue-700 shadow-md";
  }
  if (cellIndex === 4) {
    return (
      base +
...
const MiniGrid = ({
  ...
  readonly currentChart: MandalaChartType;
  readonly focusIndex: number;
  readonly onChangeFocusIndex: (index: number) => void;
  readonly onSetEditingCell: (
    cell: { readonly gridIdx: number; readonly cellIdx: number } | null
  ...
  return (
    <div
      ...
      `}
    >
      {currentChart.grids[gridIdx].map((cellText, cellIndex) => (
        <button
          key={`${gridIdx}-${cellIndex}`}
          type="button"
          className={getCellStyles(gridIdx, cellIndex, isFocusView)}
          onClick={() => {
            if (!isFocusView && !isActive) {
              onChangeFocusIndex(gridIdx);
            } else {
              onSetEditingCell({ gridIdx, cellIdx: cellIndex });
            }
          }}
          ...
          {viewMode === "full" && (
            <div className="w-full max-w-6xl animate-fade-in animate-zoom-in">
              ...
              <div className="grid grid-cols-3 gap-1 md:gap-3 lg:gap-4 aspect-square max-h-[85vh] mx-auto p-1">
                {currentChart.grids.map((_, index) => (
                  <div key={`full-${index}`} className="relative w-full h-full">
                    <MiniGrid
                      ...
                      onChangeFocusIndex={onChangeFocusIndex}
