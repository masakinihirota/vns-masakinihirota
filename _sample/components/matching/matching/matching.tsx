...
interface MatchingProperties {
  ...
  onSetMatchCriterion: (value: "count" | "score") => void;
  onSetProcessLimit: (value: number) => void;
  onSetScoreThreshold: (value: number) => void;
  onSetMatchMode: (value: "expand" | "refine") => void;
  onRunMatching: () => void;
  ...
  onMergeProfile?: (user: UserProfile) => void;
}

export const Matching: React.FC<MatchingProperties> = ({
  selectedProfileId,
  selectedCategories,
  matchCriterion,
  processLimit,
  ...
  const handleRemoveUser = (user: UserProfile) => {
    onRemoveUser(user);
    setShowToast({ show: true, user });
    setTimeout(() => {
      setShowToast((previous) => (previous.user?.id === user.id ? { show: false, user: null } : previous));
    }, 5000);
  };

  const handleUndo = () => {
    if (showToast.user) {
      onRestoreUser(showToast.user);
      setShowToast({ show: false, user: null });
    }
  };
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] bg-slate-50 text-slate-900 font-sans overflow-hidden text-base">
      {/* Toast Notification */}
      {showToast.show && (
        <div className="fixed bottom-24 right-8 z-[100] animate-in slide-in-from-bottom-4 duration-300">
          <div className="bg-slate-900 text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-4 border border-white/10">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-bold">
                {showToast.user?.name}をリストから除外しました
              </span>
            </div>
            <button
              onClick={handleUndo}
              className="text-indigo-400 hover:text-indigo-300 text-sm font-black uppercase tracking-widest flex items-center gap-1 ml-2 pl-4 border-l border-white/10"
            >
              <RotateCcw size={14} /> Undo
            </button>
            <button
              onClick={() => setShowToast({ show: false, user: null })}
                ...
                <div className="space-y-4">
                  ...
                  <div className="px-1">
                    <input
                      ...
                      onChange={(e) =>
                        matchCriterion === "count"
                          ? onSetProcessLimit(Number.parseInt(e.target.value))
                          : onSetScoreThreshold(Number.parseInt(e.target.value))
                      }
              ...
              <div className="bg-white rounded-[40px] shadow-sm border border-slate-100 overflow-hidden">
                ...
                <div className="p-8 bg-slate-50 border-t border-slate-100 flex gap-4">
                  {rightSidebarTab === "watch" ? (
                    <>
                      <button
                        onClick={() => handleRemoveUser(viewingUser)}
                        className="flex-1 py-4 bg-slate-100 text-slate-600 rounded-2xl font-bold text-base hover:bg-slate-200 transition-all flex items-center justify-center gap-2"
                      >
                        <UserMinus size={18} /> 除外する
                      </button>
                      <button
                        onClick={() => onMergeProfile?.(viewingUser)}
                        className="flex-[2] py-4 bg-indigo-600 text-white rounded-2xl font-bold text-base hover:bg-indigo-700 transition-all shadow-lg flex items-center justify-center gap-2"
                      >
                        <UserPlus size={18} /> 自分にも追加（取り込む）
                      </button>
                    </>
                  ) : (
                    <button
