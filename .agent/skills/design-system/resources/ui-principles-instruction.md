...
- **データ**: モックデータ（PostgreSQL接続前提のスキーマ互換形式）
...
│       └── types.ts          # 型定義（PostgreSQL互換）
```

### 2. モックデータの形式（PostgreSQL互換）

モックデータは将来のPostgreSQL接続を前提として、以下の形式で定義してください。

```typescript
// src/lib/mock-data/types.ts
// PostgreSQLのテーブルスキーマと互換性のある型定義

/**
 * ユーザープロファイル型
 * @description PostgreSQL profiles テーブルと互換
 */
export type MockUser = {
  ...
  bio: string | null;
  // PostgreSQLのauth.usersとの外部キー想定
};

/**
 * コミュニティ型
 * @description PostgreSQL communities テーブルと互換
 */
...
/**
 * ユーザーモックデータ
 * @description 静的UIプレビュー用。PostgreSQLスキーマ互換形式。
 */
...
- DADS実装指示書: `./ui-dadsi-instruction.md`
- Shadcn/UI指示書: `./ui-shadcn-instruction.md`
