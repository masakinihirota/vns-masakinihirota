...
2. **未完了TODOが1つでもある間は新規計画を作成しない**
   - `schedule_todo_list/` に未完了TODO（`[ ]` または進行中）が残っている場合、
     新しい計画ファイル・新しいTODOファイルは作成禁止。
   - 先に既存TODOの完了、または明示的な整理（統合・廃止・アーカイブ）を行う。

3. **計画開始前の必須チェック**
   - 新しい計画立案依頼を受けた場合でも、必ず先に `schedule_todo_list/` の残TODOを確認し、
     未完了があれば「先に既存TODOを閉じる」方針で対応する。

4. **新規TODO作成前の未完了数チェック（必須）**
  - 新規TODOを作る前に、アクティブTODO 1本の未完了チェックボックス数（`- [ ]`）を必ず数える。
  - 未完了数が 1 以上なら新規TODO作成は禁止。
  - 未完了数が 0 の場合のみ、新規TODO作成を許可。
