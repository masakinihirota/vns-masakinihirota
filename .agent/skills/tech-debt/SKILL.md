...
- **構造的重複の特定**:
  ...
  - **推奨ツール**: `jscpd` (Copy/Paste Detector)
    ```bash
    npx jscpd src --min-lines 10 --min-tokens 50 --threshold 0
    ```
  - **手動検出**: `rg` (ripgrep) を使用して、特徴的な行を検索します。
    ```bash
    rg "const handleSubmit =" src/components
    ```
...
- **未使用のインポート・変数**:
  ...
  - **推奨ツール**: `knip`
    ...
    npx knip
    ```
    ※ `knip.json` の設定が必要ですが、未使用のファイル、エクスポート、依存関係を強力に検出します。
- **コメントアウトされたコード**:
