...
- フレームワーク（Next.jsなど）のベストプラクティスに従っているか？
...
Antigravityの `.agent/skills` 内のガイドラインに従っているか確認してください。
特に `design-system` (UI/UX) や `backend-engineering` のルール遵守を徹底してください。
...
- 不要な一時ファイル（レポート/ログ）の削除
  ...
  - ワークスペースをクリーンに保つために、不要な一時ファイルは削除することを強く推奨します。
