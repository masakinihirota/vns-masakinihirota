...
2.  **セキュリティ＆パフォーマンス検証**: `code-review` スキルを活用し、以下の観点を重点的にチェックします。
    - **RSC & API Security**: Server Actionsの不適切な利用（RPC的利用）がないか、Route Handlersが適切に使われているか。Zodによる入出力のバリデーション漏れがないか。SQLインジェクション対策は十分か。
    - **Architecture**: UIとロジック（副作⽤）の分離原則が守られ、`page.tsx` で過度なデータ取得が行われていないか。
    - **Performance**: ループ内でのクエリ発行（N+1問題）、不要なカラムの取得（オーバーフェッチ）の有無。
