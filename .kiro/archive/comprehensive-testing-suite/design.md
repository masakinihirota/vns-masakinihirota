...
- **スタイリング**: Tailwind CSS, Radix UIコンポーネント, shadcn/ui
- **バックエンド**: Hono (Edge Runtime), Supabase
- **データベース**: PostgreSQL (Drizzle ORM),Supabase
- **認証**: Supabase Auth
- **国際化**: next-intl
...
**Vitest + React Testing Library + Playwright**の組み合わせを採用します：

- **Vitest**: ユニットテスト・統合テストのメインフレームワーク（高速、Viteベース）
