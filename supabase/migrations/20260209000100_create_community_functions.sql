...
CREATE OR REPLACE FUNCTION public.complete_transaction(
    ...
    p_user_id UUID -- 実行者（購入者またはシステム管理者）
) RETURNS BOOLEAN AS $$
DECLARE
    v_tx public.market_transactions%ROWTYPE;
    v_item public.market_items%ROWTYPE;
    v_receiver_root_id UUID;
BEGIN
    SELECT * INTO v_tx FROM public.market_transactions WHERE id = p_transaction_id FOR UPDATE;
    ...
    -- 受取者のルートアカウント特定
    -- seller_id が受取者
    SELECT r.id INTO v_receiver_root_id
    FROM public.root_accounts r
    JOIN public.user_profiles u ON u.root_account_id = r.id
    WHERE u.id = v_tx.seller_id;

    -- 受取者に売上加算
    UPDATE public.root_accounts SET points = points + v_tx.seller_revenue WHERE id = v_receiver_root_id;

    -- ステータス更新
    UPDATE public.market_transactions SET status = 'completed', completed_at = now() WHERE id = p_transaction_id;
...
DECLARE
    ...
    v_foundation_fee INTEGER := 1000; -- 建国コスト (ハードコード or DB参照)
    v_nation_id UUID;
BEGIN
    -- 所有者のポイント確認
