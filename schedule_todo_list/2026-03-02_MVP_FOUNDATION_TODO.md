...
### 5. RBACテスト補強と完全修正（緊急）
- [ ] 5.1 `platform_admin` / `ghost` / `persona` の境界ケース追加と完全修復（rbac-boundary-cases.test.ts: 16/16テスト成功させる）
  - **※注意: モッキング設定等の問題であっても、テストを修正し100%成功させること。**
- [x] 5.2 `active_profile_id = null` 時の安全側挙動（deny-by-default）を確認
- [ ] 5.3 DB統合テスト等のスキップ設定を外し、CI/実行環境でパスするようコードまたはモックを修正する

**完了条件**
- [x] RBAC判定でID変換ミスが発生しない
- [ ] Ghost/Persona/管理者の仕様がテストで固定化される（100%テスト成功率）
- [ ] セキュリティテストが常時グリーン（rbac-boundary-cases 含む全てで100%通過）
...
- [ ] 認証状態によってログインボタンの表示が正しく切り替わる（UIテスト通過）
- [ ] お試しボタンクリック時、バックエンドに通信せずローカル状態でTrialモードに入ることが確認できる
...
### 8. 起動時検証の追加（必須ルール適用）
- [ ] 8.1 OAuth認証情報（GOOGLE_CLIENT_ID等）の必須チェックをシステム起動時に実装
- [ ] 8.2 本番環境（`NODE_ENV=production`）での `USE_REAL_AUTH=true` 強制チェックを実装
...
- 完了: 25 / 43
- ステータス: 🚧 **テスト修正待ち**（境界ケーステストの完全修正が必要）
- 最後の修正: 2026-03-02 レビューによるタスク差し戻しと、ヘッダー追加要件に基づくUI基盤タスク（Priority 3）の追加
...
| rbac-boundary-cases.test.ts | ❌ 未完了 | 10/16 (修正必須) |
