...
### Auth.js認証システムの導入（Supabase非依存）

- **決定事項**: Auth.js v5 + Docker PostgreSQL + Drizzle Adapter による、Supabaseとは完全に独立した認証システムを導入。
- **認証方式**: GitHub OAuth + Google OAuth のみ（メール認証は使用しない）。
- **DB**: Auth.js専用PostgreSQL（Docker、ポート `5433`）。テーブル名には `authjs_` プレフィックスを使用。
- **ログインページ**: `/authjs-login`（既存の `/login` とは別）。紫色のボーダーと「Auth.js」バッジで区別。
- **理由**: Supabase依存から脱却し、将来的な認証基盤の柔軟性を確保するため。
- **反映先**: `docker-compose.authjs.yml`, `src/lib/auth/authjs.config.ts`, `src/app/(public)/authjs-login/page.tsx`
- **OAuthアプリ登録要**: GitHub/Google側にAuth.js用の新規OAuthアプリ作成が必要（コールバックURL: `/api/auth/callback/github`, `/api/auth/callback/google`）。
