...
- メール/パスワード認証でJWTをクッキーに保存（セキュア属性とHttpOnly属性を使用して、クライアントサイドスクリプトからのアクセスを防ぎ、セッションの安全性を確保します）
- ログイン用ルートを保護するためのグローバルミドルウェア
- サーバーアクションを保護するためのローカルミドルウェア、またはZodスキーマによる検証
...
また、`/sign-up` ページから新しいユーザーを作成することも可能です。
...
オプションとして、Stripe CLIを使用してローカル環境でStripeのWebhookをリッスンし、サブスクリプション変更イベントを処理できます:
...
SaaSアプリケーションを本番環境にデプロイする際は、以下の手順を実行してください:
...
Vercelプロジェクトの設定（またはデプロイ中）で、必要なすべての環境変数を追加します。本番環境用の値に更新することを忘れないでください。以下の変数を含めます:
